...
(define (expand-top env dc original-form rest*)
  (let loop ((env env) (form original-form))
    (match form
      (#`(begin . #,(and f* (list _ ...))) (expand-top* env dc f* rest*))

      (#`(define (#,(? identifier? name) . #,p*) . #,(list body ..1))
       (expand-top env dc #`(define #,name (lambda #,p* . #,body)) rest*))

      (#`(define #,(? identifier? name) #,body)
       (define i* (filter identifier? (renamed-formal-param* dc (list name))))
       (define env-rest (env-extend* env (variable-binding* i*)))
       (cons (cons name body) (expand-top* env-rest dc #'() rest*)))

      (#`(begin . #,_) (cons (cons #f (exception 'top-begin form))
                             (expand-top* env dc #'() rest*)))
      (#`(define . #,_) (cons (cons #f (exception 'top-define form))
                              (expand-top* env dc #'() rest*)))

      (_ (define expanded-form (expand-once env form))
         (if (syntax? expanded-form)
           (loop env (syntax-rename/defctx expanded-form dc))
           (cons (cons #f original-form) (expand-top* env dc #'() rest*)))))))

(define (expand-top* env dc form* rest*)
  (match (cons form* rest*)
    ((cons #'() '()) '())
    ((cons #'() `(,f* . ,f**)) (expand-top* env dc f* f**))
    ((cons #`(#,a . #,d) _) (expand-top env dc a (cons d rest*)))
...
(define (i*->expanded-body old-env body+)
  (lambda (i*)
    (define dc (defctx))
    (define renamed-form
      (syntax-rename/defctx (syntax-rename/identifier* body+ i*) dc))
    (define env (env-extend* old-env (variable-binding* i*)))
    (define definitions (expand-top* env dc renamed-form '()))
