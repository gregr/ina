...
(define (rx-interpret program get pos)
  (match-define
    `#(,state-count ,branch-count ,address-count ,name=>address ,code) program)
  (define thread-pool
    (let* ((size (+ branch-count (* state-count 2)))
           (pool (make-mvector size #f)))
      (let loop ((i (- size 1)))
        (when (<= 0 i)
  ...
  (define (thread-halt! t pos)  (mvector-set! t address-count       pos))
  (define (thread-new! t?)
  ...
  (define (visits-clear!)
    (for-each (lambda (vi) (mvector-set! visited? (mvector-ref visits vi) #f))
              (range visit-count))
    (set! visit-count 0))
  (define (continue! q q-size t pos pc)
    (cond ((visit?! pc)
           (match (vector-ref code pc)
             (`#(branch ,pc1 ,pc2)
               (define q-size2 (continue! q q-size (thread-new! t) pos pc1))
               (continue! q q-size2 t pos pc2))
             (`#(jump ,pc)       (continue! q q-size t pos pc))
             (`#(store ,address) (mvector-set! t address pos)
                                 (continue! q q-size t pos (+ pc 1)))
             (_ (thread-pc-set! t pc) (mvector-set! q q-size t) (+ q-size 1))))
          (else (thread-free! t) q-size)))

  (define halted #f)
  (define queue  (make-mvector state-count #f))
  (let loop ((pos pos) (queue      queue)
                       (queue-size (continue! queue 0 (thread-new! #f) pos 0))
                       (q2         (make-mvector state-count #f)))
    (define (consume pos char)
      (define q2-size
        (let pop ((qi 0) (q2-size 0))
          (if (< qi queue-size)
            (let* ((t (mvector-ref queue qi)) (pc (thread-pc t)))
              (match (vector-ref code pc)
                (`#(test ,valid? ,_)
                  (pop (+ qi 1) (if (not (valid? char)) q2-size
                                  (continue! q2 q2-size t pos (+ pc 1)))))
                (`#(halt)       (thread-halt! t pos) (set! halted t) q2-size)))
            q2-size)))
      (loop pos q2 q2-size queue))
    (visits-clear!)
    (if (= queue-size 0) halted
      (let decode ((pos pos))
        (define b (get pos))
        (if (not b) (consume pos #f)
          (let iter ((pos (+ pos 1)) (next (utf8->unicode b)))
            (cond ((not next)        halted)
                  ((procedure? next) (define b (get pos))
                                     (if b (iter (+ pos 1) (next b)) halted))
                  (else              (consume pos next)))))))))
