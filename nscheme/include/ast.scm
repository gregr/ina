;; TODO: use separate lib/some-library.scm programs for linking code and exporting definitions
...
(define (ast:ref         pv name)                   `#(ref         ,pv ,name))
(define (ast:if          pv ast.c ast.t ast.f)      `#(if          ,pv ,ast.c ,ast.t ,ast.f))
(define (ast:begin       pv ast.effect* ast.result) `#(begin       ,pv ,ast.effect* ,ast.result))
...
(define (ast:let         pv bpair* body)            (apply ast:call pv (ast:lambda #f (map car bpair*) body)
                                                           (map cdr bpair*)))
(define (ast:list        pv ast*)                   (apply ast:call pv (ast:lambda #f 'xs (ast:ref 'xs))
                                                           ast*))
...
(define (cenv-ref cenv.0 name)
  (let loop ((j 0) (cenv.1 cenv.0))
    (match cenv.1
      ('() (error "unbound reference" name))
      ((cons (cons rec? (cons variadic? names)) cenv)
       (let find ((i 0) (n* names))
         (match n*
           ('()         (loop (+ j 1) cenv))
           ((cons n n*) (if (equal? n name)
                          (if rec?
                            (if variadic?
...
(define (cenv-extend/rec? cenv param rec?)
  (cons (cons rec? (let loop ((param param) (names '()))
                     (cond ((null?   param) (cons #f (reverse names)))
                           ((symbol? param) (cons #t (reverse (cons param names))))
                           (else            (loop (cdr param) (cons (car param) names))))))
        cenv))
...
(define (ast-stage ast cenv)
  (let loop.full ((ast ast) (cenv cenv))
    ...
    (match ast
      ...
      (`#(ref ,_ ,name)                   (cenv-ref cenv name))
      (`#(if ,_ ,a.c ,a.t ,a.f)           (let ((s.c (loop a.c)) (s.t (loop a.t)) (s.f (loop a.f)))
                                            (lambda (renv) (if (s.c renv) (s.t renv) (s.f renv)))))
      (`#(begin ,_ ,a*.effect ,a.result)  (let ((s*.effect (map loop a*.effect))
                                                (s.result  (loop a.result)))
                                            (lambda (renv)
                                              (for-each (lambda (s) (s renv)) s*.effect)
                                              (s.result renv))))
