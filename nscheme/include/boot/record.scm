(define (make-rtd name parent final? field*)
  (when (and parent (rtd-final? parent))
    (violation "rtd parent is final" name parent final? field*))
...
(define (rtd-final?         rtd) (vector-ref    rtd 2))
(define (rtd-field*         rtd) (vector-ref    rtd 3))
...
(define (record-field-position-accessor rtd i)
  (let ((count.field* (vector-length (rtd-field* rtd))))
    (unless (and (integer? i) (<= 0 i) (< i count.field*)) (violation "not a field position" rtd i))
...
(define (record-field-name->position rtd name)
  (let* ((field*       (rtd-field* rtd))
    ...
    (let loop ((i 0))
      (cond ((= i count.field*)                (violation "not a field name" rtd name))
