      ...
      (cond
         ...
         (let ((P.a (pattern:cons-car P)) (P.d (pattern:cons-cdr P)))
           (if (pattern:ellipsis? P.a)
               (let ((P.a (pattern:ellipsis-inner P.a)))
                 (let check ((P P.d) (rP* '()) (id* id*))
                   (cond ((pattern:cons? P)
                          (let ((P.next (pattern:cons-car P)))
                            (when (pattern:ellipsis? P.next)
                              (raise-syntax-error "too many ellipses" P.next))
                            (let-values (((P.next id*) (loop P.next id*)))
                              (check (pattern:cons-cdr P) (cons P.next rP*) id*))))
                         (else (let*-values (((P   id*) (loop P   id*))
                                             ((P.a id*) (loop P.a id*)))
                                 (values (wrap ($p:rlist/ellipsis P rP* P.a)) id*))))))
        ...
        ((pattern:vector? P)
         (let* ((P*  (pattern:vector-element* P))
           ...
           (cond ((pair? Pe*)
                  (when (pair? (cdr Pe*)) (raise-syntax-error "too many ellipses" P))
                    ...
                    (cond ((pattern:ellipsis? P)
                           (let-values (((P id*) (loop (pattern:ellipsis-inner P) id*)))
                             (let vloop ((P* P*) (rP* '()) (id* id*))
                               (cond ((null? P*) (values (wrap ($p:vector/ellipsis
        ...
        (else             (raise-syntax-error "unsupported pattern"
                                              (syntax-provenance-set (ast-provenance-set P #f)
                                                                     (ast-provenance P))))))))
            ...
            (else
              (let ((clause (syntax-unwrap (car stx*.clause*))))
                ...
                (let*-values (((stx.body) (cdr clause))
                              ((P id*.P)  (pattern->linear-pattern&variable*
