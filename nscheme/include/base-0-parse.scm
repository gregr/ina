...
(define (pattern-compile-linear P)
  ...
  (define ($p:rlist/ellipsis P.tail rP*.suffix P.ellipsis)
    (vector 'rlist/ellipsis P.tail rP*.suffix P.ellipsis))
  (define ($p:vector/ellipsis P*.prefix P.ellipsis P*.suffix)
    (vector 'vector/ellipsis P*.prefix P.ellipsis P*.suffix))
  (define (simplify P)
    (ast-provenance-add
      (cond
        ((or (pattern:any? P) (pattern:none? P) (pattern:var? P) (pattern:?? P)) P)
        ((pattern:quote?  P) (let loop ((stx.value (pattern:quote-value-syntax P)))
                               (let ((q (syntax-unwrap     stx.value)))
                                 ($provenance
                                   (cond ((pair?   q) ($$p:cons (loop (car q)) (loop (cdr q))))
                                         ...
                                         (else        ($$p:quote q)))
                                   stx.value))))
        ((pattern:cons?   P) (let ((P.a (pattern:cons-car P)) (P.d (pattern:cons-cdr P)))
                               (if (pattern:ellipsis? P.a)
                                   (let loop ((P P.d) (rP* '()))
                                     (cond
                                       ((pattern:cons? P)
                                        (let ((P.next (pattern:cons-car P)))
                                          (when (pattern:ellipsis? P.next)
                                            (raise-syntax-error "too many ellipses" P.next))
                                          (loop (pattern:cons-cdr P) (cons P.next rP*))))
                                       (else
                                         ($p:rlist/ellipsis P rP* (pattern:ellipsis-inner P.a)))))
                                   ($$p:cons (simplify P.a) (simplify P.d)))))
        ((pattern:vector? P) (let* ((P*  (pattern:vector-element* P))
                               ...
                               (cond ((pair? Pe*)
                                      ...
                                      (let loop ((P (car P*)) (P* (cdr P*)) (prefix* '()))
                                        (cond ((pattern:ellipsis? P)
                                               ($p:vector/ellipsis
                                                 (reverse prefix*) (pattern:ellipsis-inner P) P*))
                                              (else (loop (car P*) (cdr P*) (cons P prefix*))))))
                                     (else ($$p:vector (map simplify P*))))))
        ((pattern:app?    P) ($p:app (pattern:app-procedure P) (simplify (pattern:app-inner P))))
        ((pattern:and?    P) ($p:and (simplify (pattern:and-left P))
                                     (simplify (pattern:and-right P))))
        ((pattern:or?     P) ($p:or  (simplify (pattern:or-left P))
                                     (simplify (pattern:or-right P))))
        ((pattern:not?    P) ($p:not (simplify (pattern:not-inner P))))
        (else                (raise-syntax-error "unsupported pattern" (syntax-provenance-set
                                                                         (ast-provenance-set P #f)
                                                                         (ast-provenance P)))))
      (ast-provenance P)))
  ...
  (let loop ((allow-variables? #t) (P (simplify P)))
    (wrap
      ...
      (cond
        ...
        ((pattern:app?  P) ($$app (pattern:app-procedure P)
                                  (loop allow-variables? (pattern:app-inner P))))
        ((pattern:and?  P) ($$and (loop allow-variables? (pattern:and-left P))
                                  (loop allow-variables? (pattern:and-right P))))
        ((pattern:or?   P) ($$or (loop #f (pattern:or-left P)) (loop #f (pattern:or-right P))))
        ((pattern:not?  P) (let ((^p (loop #f (pattern:not-inner P))))
                             (lambda (succeed fail $x env) (^p fail succeed $x env))))
        ;((ast-tagged? P 'rlist/ellipsis)
        ; ;; TODO: improper-reverse $x, match P.tail, then rP*.suffix,
        ; ;; then match the rest against P.ellipsis to build up a list of results.
        ; )
        ;((ast-tagged? P 'vector/ellipsis)
