...
(define (find-file/directory* dir* name)
  (let ((name (if (string? name) (string->bytevector name) name)))
    (if (and (< 0 (bytevector-length name))
             (let ((b0 (bytevector-ref name 0)))
               (or (= b0 (bytevector-ref #"." 0)) (= b0 (bytevector-ref #"/" 0)))))
        (bytevector->string name)
        (ormap (lambda (dir)
                 (let ((path (bytevector->string (bytevector-join #"/" dir name))))
                   (and (file-type/k path (lambda _ #f) values) path)))
               (map (lambda (dir) (if (string? dir) (string->bytevector dir) dir)) dir*)))))
