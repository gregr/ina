...
(define (mvector->string mv)
  (utf8->string (list->bytevector (vector->list (mvector->vector mv)))))
...
(define (read-number/symbol get start k ksymbol)
  ...
  (define (NumberBody pos ex radix sign prev mode)
    ...
    (define (Float sign whole frac start pos ex)
      (Exp (or sign 1) (+ whole (* frac (expt radix (- start pos)))) pos ex))
    (define (Exp sign real pos ex)
      (define (Real r pos ex)
        (define real (* sign r))
        (cond ((eq? ex 'inexact) (Suffix (* sign (exact->inexact r)) pos ex))
              ((or (eqv? real +inf.0) (eqv? real -inf.0) (eqv? real +nan.0))
               (k 'error "no exact representation" start pos))
              (else (Suffix real pos ex))))
      (define (Rest esign pos) (let/cps _ (e ep) (Natural pos _)
                                 (if (eqv? ep pos) (ksymbol ep)
                                   (Real (* real (expt radix (* esign e))) ep
                                         (or ex 'inexact)))))
      (case/char (get pos)
    ...
    (define (LHS n np)
      (case/char (get np)
        ...
        ("/" (let/cps _ (d dp) (Natural (+ np 1) _)
               (define v (if (eqv? d 0) (if (eqv? n 0) +nan.0 +inf.0) (/ n d)))
               (Exp (or sign 1) v dp ex)))
        (else (Exp (or sign 1) n np ex))))
    (Natural pos (lambda (n np) (if (eqv? np pos) (NoLHS) (LHS n np)))))
...
;(define (char c)
  ...
  ;(and (eqv? (vector-length v) 1) (vector-ref v 0)))
;(define (digit->nat ch)
