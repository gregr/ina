...
(splicing-local
  ...
  (define (D->E D) (D->E/E-eval D #f))
  (define (D->E/E-eval D E-eval)
    (mlet ((rdef* '()) (^E.current #f))
      ...
      (let ((def* (reverse rdef*)) (^E (or ^E.current $values)))
        (if (null? rdef*)
            (if E-eval (call-with-values (E-eval ($thunk (^E))) $quote-values) (^E))
            (let* ((id*   (map definition-id   def*))
                   (env*  (map definition-env  def*))
                   (^rhs* (map definition-^rhs def*))
                   (^E    (if E-eval
                              (lambda () (let ((E.^body ($thunk (^E))))
                                           (apply $list E.^body (map parse-expression env* id*))))
                              ^E))
                   (E     ($letrec id* (lambda E*
                                         (for-each env-set-variable! env* id* E*)
                                         (values (map (lambda (^E) (^E)) ^rhs*) (^E))))))
              (if E-eval
                  (let ((result* (E-eval E)))
