...
(define (parse env expr)
  (define (default) (((env-ref env vocab.expression '||) env (datum->syntax #f `(#f ,expr)))))
  (define (operate e.op)
  ...
  (cond ((identifier? expr) (operate expr))
        (else (let ((x (syntax-unwrap e)))
                (cond ((pair?              x) (let ((e.op (car x)))
                      ...
                      (else                   (raise-syntax-error expr "not an expression")))))))

(define (parse-unbound-identifier env e)
  ;; TODO: first look for a restart to invoke
  (raise-syntax-error e "unbound identifier"))

(define (parse-default env _ e)
  (define (fail) (raise-syntax-error e "not a literal or procedure application form"))
    ...
    (cond ((syntax->list? expr) => (lambda (e*)
                                     (let ((argc (- (length e*) 1)))
                                       (unless (<= argc.min argc)
                                         (raise-syntax-error expr "not few keyword arguments"))
                                       (unless (<= argc (or argc.max argc))
                                         (raise-syntax-error expr "too many keyword arguments"))
                                       (ast-provenance-add (apply parser env e*) pv))))
          (else (raise-syntax-error expr "not a keyword application form")))))
...
(define (parse-case-lambda env _ . e*.cc)
  (define (fail) (raise-syntax-error e.cc "not a case-lambda clause"))
                ...
                (let ((e*~.param (syntax->improper-list (car e*)))
                  ...
                  (let ((e*.param (improper-list->list e*~.param)))
                    (unless (andmap identifier? e*.param)
                      (raise-syntax-error (car e*) "formal parameter names must be identifiers"))
                    (when (null? e*.body)
                      (raise-syntax-error e.cc "case-lambda clause cannot have an empty body"))
                    (let ((id* (map identifier-id e*.param)))
                      (unless (unique? id*)
                        (raise-syntax-error (car e*) "duplicate formal parameter names"))
                      (let* ((param (improper-list-map (lambda (e) (fresh-address (identifier-id e)))
                        ...
                        (let ((env.scope (env:scope)))
                          ...
                          (env-set!**  env.scope vocab.expression
                                       id* (map parse-variable-ref/address addr*))
                          (case-clause param (parse-body (env:inherit env env.scope) e*.body)))))))))
          (else (fail))))
  (ast:case-lambda #f (map (lambda (e.cc) (parse-case-lambda env e.cc)) e*.cc)))
  ...
  (let* ((id*          (mutable-set))
    ...
    (lambda (method)
      (case method
        ((id-add!)               (lambda (e.id)
                                   (let ((id (identifier-id e.id)))
                                     (when (set-member? id* id)
                                       (raise-syntax-error e.id "name defined multiple times"))
                                     (set-add! id* id))))
        ...
        ((env-full-push!)        (set! env*.full  (cons env.full env*.full))
                                 (set! env.full   (env:inherit env.full env.scope)))
...
(define (defstate-binding-pair-add!     dst addr ^ast) ((dst 'binding-pair-add!) addr ^ast))
  ...
  (let loop ((e.lhs e.lhs) (e.rhs e.rhs))
    (cond ((identifier? e.lhs)
           ...
           (let* ((id        (identifier-id e.lhs))
             ...
             (env-bind! env.scope id addr)
             (env-set!  env.scope vocab.expression id (parse-variable-ref/address addr))
          ...
          (else (let ((x (syntax-unwrap e.lhs)))
                  (cond ((pair? x) (loop (car x)
                        ...
                        (else      (raise-syntax-error e.lhs "not a definable form"))))))))
...
(define (parse-splicing-local-definition dst _ local-body . stx*)
  (cond ((syntax->list? local-body)
        ...
        (else (raise-syntax-error local-body "not a list of definitions"))))
...
(define (parse-splicing-let-syntax-definition
          )
  )
...
(define (parse-invalid-expression env e)
  (raise-syntax-error e "cannot be used in an expression context"))

(define (parse-binding-pair e.bpair)
  (cond ((syntax->list? e.bpair)
         => (lambda (e*)
              (unless (= (length e*) 2)
                (raise-syntax-error e.bpair "binding pair list length must be 2"))
              (unless (identifier? (car e*))
                (raise-syntax-error e.bpair "left element of binding pair must be an identifier"))
              (cons (car e*) (cadr e*))))
        (else (raise-syntax-error e.bpair "not a binding pair"))))

(define (parse-binding-pairs e.bpairs)
  (cond ((syntax->list? e.bpairs) => (lambda (e*) (map parse-binding-pair e*)))
        (else (raise-syntax-error e.bpairs "not a list of binding pairs"))))
...
;; TODO: automate populating with parse-invalid-expression
(define env.initial.expression.invalid
