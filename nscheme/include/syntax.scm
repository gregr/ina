...
(define (env:inherit env.next env.first)
  (lambda (method)
    (case method
      ...
      ((ref)     (lambda (vocab id) (or (env-ref env.first vocab id)
                                        (env-ref env.next  vocab id))))
...
(define (env:unmark env m)
  (define (apply-unmarked f id) (and (identifier? id)
                                     (let ((id (syntax-unmark id m)))
                                       (and id (f id)))))
  (lambda (method)
    (case method
      ((address)    (lambda (id)       (apply-unmarked (lambda (id) (env-address env id)))))
      ((ref)        (lambda (vocab id) (apply-unmarked (lambda (id) (env-ref env vocab id)))))
      ((bind! set!) (error "invalid immutable environment operation" method))
      (else         (error "invalid environment operation"           method)))))
...
(define (env-ref     env vocab id)       ((env 'ref)  vocab id))
(define (env-bind!   env       id  addr) ((env 'bind!)      id addr))
(define (env-set!    env vocab id  v)    ((env 'set!) vocab id v))

(define (env-ref*    env vocab ids)       (map (lambda (id) (env-ref env vocab id)) ids))
(define (env-set!**  env vocab ids vs)    (for-each (lambda (id v) (env-set! env vocab id v)) ids vs))
(define (env-set!*   env vocab . idvs)    (let ((idvs (plist->alist idvs)))
                                            (env-set!** env vocab (map car idvs) (map cdr idvs))))
(define (env-bind**  env       ids addrs) (for-each (lambda (id addr) (env-bind! env id addr)) ids addrs))
(define (env-bind*   env       . idvs)    (let ((idvs (plist->alist idvs)))
                                            (env-bind** env (map car idvs) (map cdr idvs))))
