...
(define (utf8-ref/b0&width/k bv i b0 width kf k)
  (let ((kf (lambda x* (apply kf "utf8 reference at" bv i x*))))
    (case width
      ((1) (k b0))
      ((2) (if (< (+ i 1) (bytevector-length bv))
               (utf8-decode-2/k b0 (bytevector-ref bv (+ i 1)) kf k)
               (kf "missing 2-byte utf8 continuation" (bytevector-length bv))))
      ((3) (if (< (+ i 2) (bytevector-length bv))
               (utf8-decode-3/k b0 (bytevector-ref bv (+ i 1)) (bytevector-ref bv (+ i 2)) kf k)
               (kf "missing 3-byte utf8 continuation" (bytevector-length bv))))
      ((4) (if (< (+ i 3) (bytevector-length bv))
               (utf8-decode-4/k b0 (bytevector-ref bv (+ i 1)) (bytevector-ref bv (+ i 2))
                                (bytevector-ref bv (+ i 3)) kf k)
               (kf "missing 4-byte utf8 continuation" (bytevector-length bv))))
...
(define (utf8-index bv n) (let ((len (bytevector-length bv)))
                            (let loop ((i 0) (count 0))
                              (cond ((<= n count) i)
                                    ...
                                    (else (error "utf8-index count too large" bv n count))))))
