...
(splicing-local
  ((define byte:bell 7)
   ...
   (define byte:feed 12) (define byte:return 13) (define byte:escape 27) (define byte:! 33)
   (define byte:dquote 34) (define byte:# 35) (define byte:quote 39) (define byte:lparen 40)
   (define byte:rparen 41) (define byte:+ 43) (define byte:comma 44) (define byte:- 45)
   (define byte:. 46) (define byte:/ 47) (define byte:0 48) (define byte:8 56)
  ...
  (define (make-denotate notation)
    (let ((utf8->n (make-utf8->number
      ...
      (lambda (reader port)
        (mlet ((buf (make-mbytevector 8192 0)) (pos.buf 0) (end.buf 0) (eof? #f) (pos.abs 0))
          (define (pos++ width) (let ((pos pos.abs))
                                  (set! pos.abs (+ pos width))
                                  (set! pos.buf (+ pos.buf width))
                                  pos))
          (define (rerror pos offset blame desc x*)
            (reader-error reader pos offset blame (if (null? x*) desc (cons desc x*))))
          (define (fail-local offset blame desc . x*) (rerror pos.abs offset blame desc x*))
          (define (fail       offset blame desc . x*) (rerror (pos++ offset) offset blame desc x*))
          ...
          (define (peek offset k)
            (define (kf . x*) (apply fail offset #f x*) #f)
            (let loop ()
          ...
          (define (atom size x) (reader-atom reader (pos++ size) size x))
          (define (lbrack width shape type len)
            (reader-left-bracket reader (pos++ width) width shape type len))
          (define (rbrack width shape) (reader-right-bracket reader (pos++ width) width shape))
          (define (prefix width type) (reader-prefix reader (pos++ width) width type))
          (define (EOF pos) (reader-eof reader pos))
          (define (Line) (reader-newline reader (pos++ 1)))
          (define (Space width) (pos++ width) (Any))
          (define (Dot)
            (peek 1 (lambda (c w)
                      (if (separator? c) (reader-dot reader (pos++ 1) 1) (Number^Symbol)))))
          (define (Comment:datum) (reader-datum-comment-prefix reader (pos++ 2) 2))
          (define (Comment:block)
            (let loop ((i 2) (level 0))
              (seq-alt* i (lambda () (peek i (lambda (c width) (and (or (not (eqv? c byte:newline))
                                                                        (reader-newline reader i))
                                                                    (loop (+ i width) level)))))
                        (list byte:pipe byte:#)
                        (lambda () (let ((i (+ i 2))) (if (< 0 level)
                                                          (loop i (- level 1))
                                                          (reader-comment reader (pos++ i) i))))
                        (list byte:# byte:pipe) (lambda () (loop (+ i 2) (+ level 1))))))
          (define (Comment:line)
            (let ((start (pos++ 1)))
              (let loop () (peek 0 (lambda (c width)
                                     (let ((end (pos++ width)))
                                       (if (or (not c) (eqv? c byte:newline))
                                           (and (reader-comment reader start (- end start)) (Any))
                                           (loop))))))))
          (define (Boolean x) (and (atom 2 x)
                                   (peek 0 (lambda (c w) (or (separator? c)
                                                             (fail 0 c "not a separator"))))))
          (define Text
            (mlet ((buf.text (make-mbytevector 64 0)))
              (lambda (i byte:delim text/k)
                (mlet ((pos.text 0) (fail? #f))
                  ...
                  (define (push c) (let* ((w (utf8-encode-width c)) (pos pos.text) (end (+ pos w)))
                                     ...
                                     (set! pos.text end)))
                  (let loop ((i i))
                    (define (fail/next i . x*) (set! fail? #t) (and (apply fail-local i x*)
                                                                    (loop i)))
                      ...
                      (next-separator
                        i (lambda (i.sep c.sep)
                            (if (eqv? c.sep byte:semicolon)
                                (if (= i.sep i)
                                    (fail/next i.sep #f "missing escape code")
                                    (let ((pos pos.buf))
                                      (let loop ((c 0) (i i))
                                        (if (< i i.sep)
                                            (let* ((b (mbytevector-ref buf (+ pos i)))
                                              ...
                                              (if (< -1 x 16)
                                                  (loop (+ (* c 16) x) (+ i 1))
                                                  (fail/next i b "not a base-16 digit")))
                                            (k (+ i.sep 1) c)))))
                                (fail/next i.sep c.sep "not an escape code terminator")))))
                    (define (k.escape c _)
                      (cond ((eqv? c byte:newline)   (and (reader-newline reader i)
                                                          (loop (+ i 2))))
                            ...
                            ((eqv? c byte:u)
                             (decode-escape
                               (+ i 2) (lambda (next c)
                                         (if (unicode? c)
                                             (push/next next c)
                                             (fail/next (+ i 2) c "invalid escaped codepoint")))))
                            ((eqv? c byte:x)
                             (decode-escape
                               (+ i 2) (lambda (next b)
                                         (if (<= 0 b 255)
                                             (begin (push-byte b) (loop next))
                                             (fail/next (+ i 2) c "invalid escaped byte")))))
                            (else (fail/next (+ i 1) c "invalid escape sequence"))))
                    (define (k.next c w)
                      (cond ((not c) (fail i #f "EOF before text delimiter" byte:delim))
                            ((eqv? c byte:backslash) (peek (+ i 1) k.escape))
                            ((eqv? c byte:delim)
                             (if fail?
                                 (fail (+ i 1) #f "invalid text")
                                 (text/k (mbytevector->bytevector buf.text 0 pos.text)
                                         (lambda (x) (atom (+ i 1) x)))))
                            (else (and (or (not (eqv? c byte:newline)) (reader-newline reader i))
                                       (push/skip w c)))))
                    (peek i k.next))))))
          (define (Bytevector i len)
            (Text i byte:dquote
                  (if len
                      (lambda (text k)
                        (let ((len.bv (bytevector-length text)))
                          (cond ((< len len.bv) (fail i #f "bytevector longer than length-prefix"
                                                      (cons 'length        len.bv)
                                                      (cons 'length-prefix len)))
                                ((< len.bv len)
                                 (if (< 0 len.bv)
                                     (let* ((b.pad (bytevector-ref text (- len.bv 1)))
                                            (mbv   (make-mbytevector len b.pad)))
                                       (mbytevector-copy! mbv 0 text 0 (- len.bv 1))
                                       (k (mbytevector->bytevector mbv)))
                                     (fail i #f "empty length-prefixed bytevector" len)))
                                (else (k text)))))
                      (lambda (text k) (k text)))))
          (define (String)      (Text 1 byte:dquote (lambda (text k) (k (utf8->string text)))))
          (define (Symbol/pipe) (Text 1 byte:pipe   (lambda (text k) (k (utf8->symbol text)))))
          (define (Number^Symbol)
            (next-separator-pos 1 (lambda (i.sep)
                                    (let ((text (mbytevector->bytevector buf pos.buf i.sep)))
              ...
              (peek i (lambda (c width)
                        (let ((j (+ i width)) (n (undigit10 c)))
                          (cond (n                     (loop j (+ (* len 10) n)))
                                ...
                                (else (seq-alt*
                                        i (lambda () (fail i c "misplaced length-prefix"))
          ...
          (define (Hash)
            (peek 1 (lambda (c _)
                      (cond ((memv c (list byte:F byte:f)) (Boolean #f))
                            ...
                            (else (seq-alt* 1 (lambda () (fail 1 c "invalid #"))
                                            (list byte:v byte:u byte:8 byte:lparen)
          ...
          (define (Any)
            (peek 0 (lambda (c width)
                      (cond ((not c)                 (EOF pos.abs))
                            ((eqv? c byte:newline)   (Line))
                            ((unicode-space? c)      (Space width))
                            ...
                            ((eqv? c byte:#)         (Hash))
                            ((eqv? c byte:.)         (Dot))
                            ((eqv? c byte:semicolon) (Comment:line))
