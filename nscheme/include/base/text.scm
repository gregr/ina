...
(define (reader:datum) (reader:datum/annotate (lambda (loc text x) x)))
(define (reader:datum/annotate annotate)
  (mlet ((state #f) (frame* '()))
    (define (make-prefix loc text type) (cons 'prefix (annotate loc text type)))
      ...
      (lambda (loc text datum)
        (let ((top (frame*-top)))
          (cond ((eqv? top 'rb) (fail loc text "too many values following dot"))
                ...
                (else (push-datum (annotate loc text datum))))))
      ...
      (lambda (loc text shape)
        ...
        (let ((top (frame*-top)))
          (cond ((eqv? top 'rb)
                 (frame*-pop)
                 (let ((top (frame*-top)))
                   (check-shape top (lambda ()
                                      (let ((x* (seq-x* top)))
                                        (frame*-pop)
                                        (let loop ((x (car x*)) (x* (cdr x*)))
                                          (if (null? x*)
                                              (push-datum (annotate (seq-loc top) (seq-text top) x))
                 ...
                 (check-shape
                   top (lambda ()
                         (define (go x*)
                           (let ((x* (reverse x*)))
                             (frame*-pop)
                             (push-datum (annotate (seq-loc top) (seq-text top)
                                                   (case (seq-type top)
