...
(define (reader-newline              r loc codepoint)           ((vector-ref r 8)  loc codepoint))
              ...
              (seq-alt*
                i (lambda ()
                    (peek i (lambda (c width)
                              (if c
                                  (and (or (not (unicode-vspace? c)) (reader-newline reader i c))
                                       (loop (+ i width) level))
              ...
              (lambda (i byte:delim return/k)
                (mlet ((pos.text 0) (fail? #f))
                  ...
                  (let loop ((i i))
                    ...
                    (define (k.escape c w)
                      (cond ((unicode-vspace? c)     (and (reader-newline reader (+ i 1) c)
                                                          (loop (+ i 1 w))))
                            ...
                            ((eqv? c byte:delim)
                             (if fail?
                                 ...
                                 (return/k (+ i 1) (mbytevector->bytevector buf.text 0 pos.text)
                                           (lambda (x) (atom (+ i 1) x)))))
                            (else (and (or (not (unicode-vspace? c)) (reader-newline reader i c))
                                       (push/skip w c)))))
                    (peek i k.next))))))
