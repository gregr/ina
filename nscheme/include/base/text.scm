...
(splicing-local
  ((define (make-gbnode prev) (let ((gbn (mvector prev #f #t)))
   ...
   (define (gbnode-active?   gbn)      (mvector-ref  gbn 2))
   ...
   ;; TODO: store an incrementally computed segment size to eliminate placement objects
   (define (make-seg t*) (vector t*))
   (define (seg-t*        s) (vector-ref s 0))
  ...
  (define (layout:compact printer width)
    (mlet ((single-line-group-depth 1)
      ...
      (define (group-push)
        (set! start* (cons pos.actual start*))
...
#;(let ((example
  ...
  (oport-write-byte standard-output-port 10)
  (oport-write-byte standard-output-port 10)
  (notate (writer:layout (layout:compact (printer:port standard-output-port) 80)) example)
  (oport-write-byte standard-output-port 10)
  (oport-write-byte standard-output-port 10)
  (notate (example-writer/sgr (layout:compact (printer:port standard-output-port) 80)) example)
  (oport-write-byte standard-output-port 10)
  (oport-write-byte standard-output-port 10)
  (oport-write-byte standard-output-port 10)
  (verbose-notate (writer:layout (layout:compact (printer:port standard-output-port) 80)) example)
  (oport-write-byte standard-output-port 10)
  (oport-write-byte standard-output-port 10)
  (verbose-notate (example-writer/sgr (layout:compact (printer:port standard-output-port) 80)) example)
  (oport-write-byte standard-output-port 10)
  (oport-write-byte standard-output-port 10)
  )
