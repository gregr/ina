...
(splicing-local
  ((define byte:dquote    34)
   ...
   (define (mbytevector-base16-encode! mbv i c dcount)
     (case dcount
       ((1) (mbytevector-base16-encode-1! mbv i c))
       ((2) (mbytevector-base16-encode-2! mbv i c))
       ((3) (mbytevector-set! mbv i       (digit16 (bitwise-asr c 8)))
            (mbytevector-set! mbv (+ i 1) (digit16 (bitwise-and (bitwise-asr c 4) #b1111)))
            (mbytevector-set! mbv (+ i 2) (digit16 (bitwise-and c #b1111))))
       ((4) (mbytevector-set! mbv i       (digit16 (bitwise-asr c 12)))
            (mbytevector-set! mbv (+ i 1) (digit16 (bitwise-and (bitwise-asr c 8) #b1111)))
            (mbytevector-set! mbv (+ i 2) (digit16 (bitwise-and (bitwise-asr c 4) #b1111)))
            (mbytevector-set! mbv (+ i 3) (digit16 (bitwise-and c #b1111))))
       ((5) (mbytevector-set! mbv i       (digit16 (bitwise-asr c 16)))
            (mbytevector-set! mbv (+ i 1) (digit16 (bitwise-and (bitwise-asr c 12) #b1111)))
            (mbytevector-set! mbv (+ i 2) (digit16 (bitwise-and (bitwise-asr c 8) #b1111)))
            (mbytevector-set! mbv (+ i 3) (digit16 (bitwise-and (bitwise-asr c 4) #b1111)))
            (mbytevector-set! mbv (+ i 4) (digit16 (bitwise-and c #b1111))))
       (else (mbytevector-set! mbv i       (digit16 (bitwise-asr c 20)))
             (mbytevector-set! mbv (+ i 1) (digit16 (bitwise-and (bitwise-asr c 16) #b1111)))
             (mbytevector-set! mbv (+ i 2) (digit16 (bitwise-and (bitwise-asr c 12) #b1111)))
             (mbytevector-set! mbv (+ i 3) (digit16 (bitwise-and (bitwise-asr c 8) #b1111)))
             (mbytevector-set! mbv (+ i 4) (digit16 (bitwise-and (bitwise-asr c 4) #b1111)))
             (mbytevector-set! mbv (+ i 5) (digit16 (bitwise-and c #b1111)))))))
    ...
    (let* ((notation            (notation-override notation.default notation))
      ...
      (lambda (writer x)
        (let notate ((x x))
          ...
          (define (notate-text t.prefix bv len)
            (let loop ((i 0) (size 0))
              (if (< i len)
                    ...
                    (let loop ((i 0) (j (+ len.t.prefix 1)))
                      ...
                      (define (code-escape width prefix c)
                        (let ((dcount (digit16-count c)) (len.prefix (bytevector-length prefix)))
                          ...
                          (mbytevector-base16-encode! mbv (+ j len.prefix) c dcount)
             ...
             (let* ((len      (bytevector-length x))
               ...
               (if bytevector-numeric?
                   (begin (left-bracket (bytevector-append t.prefix #"vu8" text.left-bracket) x)
                          ...
                          (right-bracket))
                   (notate-text t.prefix x len))))
            ((string? x) (let ((bv (string->utf8 x))) (notate-text #"" bv (bytevector-length bv))))
            ((symbol? x)
