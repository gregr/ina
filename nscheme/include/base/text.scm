...
(define (reader-datum/k r kf keof k)
  (let ((st (reader-state r)))
    (cond ((eqv? st       'eof)   (keof))
          ((not (pair? st))       (if (not st)
                                      (error "incomplete read")
                                      (error "unknown reader state" st)))
          ((eqv? (car st) 'datum) (k  (cdr st)))
          ((eqv? (car st) 'error) (kf (cdr st)))
          (else                   (error "unknown reader state" st)))))
...
(define (read/reader/port/k r port kf keof k) (denotate r port) (reader-datum/k r kf keof k))
...
#;(let* ((out standard-output-port)
  ...
  (let* ((x->write (lambda (x)
    ...
    (example-write (read/reader/port/k (reader:datum) (iport:bytevector text.example) raise error values))
    (oport-write-byte standard-output-port 10)
    (example-write (read/reader/port/k (reader:datum) (iport:bytevector text.another-example) raise error values))
    (oport-write-byte standard-output-port 10)
    ;(example-write (read/reader/port/k (reader:datum) (iport:bytevector text.failure-example) raise error values))
    ))
