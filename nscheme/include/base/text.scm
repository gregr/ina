    ...
    (let ((utf8->n (make-utf8->number
      ...
      (lambda (reader port)
        (mlet ((buf (make-mbytevector 8192 0)) (pos.buf 0) (end.buf 0) (eof? #f) (pos.abs 0))
          ...
          (define (Line width) (reader-newline reader (pos++ width)))
              ...
              (seq-alt*
                i (lambda ()
                    (peek i (lambda (c width)
                              (if c
                                  (and (or (not (unicode-vspace? c)) (reader-newline reader i))
                                       (loop (+ i width) level))
          ...
          (define (Comment:line)
            (let loop ((i 1)) (peek i (lambda (c w) (if (or (not c) (unicode-vspace? c))
                                                        (comment i)
                                                        (loop (+ i w)))))))
          (define (Boolean x)
            (peek 2 (lambda (c w)
              ...
              (lambda (i byte:delim return/k)
                (mlet ((pos.text 0) (fail? #f))
                  ...
                  (let loop ((i i))
                    ...
                    (define (k.escape c w)
                      (cond ((unicode-vspace? c)     (and (reader-newline reader (+ i 1))
                                                          (loop (+ i 1 w))))
                            ...
                            ((eqv? c byte:delim)
                             (if fail?
                                 ...
                                 (return/k (+ i 1) (mbytevector->bytevector buf.text 0 pos.text)
                                           (lambda (x) (atom (+ i 1) x)))))
                            (else (and (or (not (unicode-vspace? c)) (reader-newline reader i))
                                       (push/skip w c)))))
                    (peek i k.next))))))
