                   ...
                   (let loop ((i 0) (size 0))
                     (if (< i len)
                         (loop (+ i 1) (+ (case (bytevector-ref x i)
                                            ...
                                            (=> (lambda (b) (cond ((unicode-control? b)
                                                                   (if (< b 16) 4 5))
                                                                  ((< b 128) 1)
                                                                  (else      5)))))
                                          size))
                         (atom (let* ((len.t.prefix (bytevector-length t.prefix))
                                 ...
                                 (let loop ((i 0) (j (+ len.t.prefix 1)))
                                   ...
                                   (if (< i len)
                                       (case (bytevector-ref x i)
                                         ...
                                         (=> (lambda (b)
                                               (define (code-escape-1)
                                                 (mbytevector-copy! mbv j #"\\x" 0 2)
                                                 (mbytevector-set! mbv (+ j 3) byte:semicolon)
                                                 (mbytevector-base16-encode-1! mbv (+ j 2) b)
                                                 (loop (+ i 1) (+ j 4)))
                                               (define (code-escape-2)
                                                 (mbytevector-copy! mbv j #"\\x" 0 2)
                                                 (mbytevector-set! mbv (+ j 4) byte:semicolon)
                                                 (mbytevector-base16-encode-2! mbv (+ j 2) b)
                                                 (loop (+ i 1) (+ j 5)))
                                               (cond ((unicode-control? b)
                                                      (if (< b 16) (code-escape-1) (code-escape-2)))
