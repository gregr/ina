...
(define (make-notate notation)
  (define (digit16 h) (if (< h 9) (+ h 48) (+ h 55)))
        ...
        (cond
          ...
          ((bytevector? x)
           (let* ((len        (bytevector-length x))
                  (prefix     (bytevector-append #"#" (if length-prefix? (number->utf8 len) #"")))
             ...
             (if bytevector-numeric?
                 (begin (left-bracket (bytevector-append prefix #"vu8" text.left-bracket) x)
                        (unless (= len 0)
                   ...
                   (if (< i len)
                       (loop (+ i 1) (+ (case (bytevector-ref x i)
                                          ...
                                          (=> (lambda (b) (cond ((unicode-control? b) 6)
                                                                ...
                                                                (else                 6)))))
                                        size))
                       (atom (bytevector-append
                               prefix
                               (if (= size len)
                                   x
                                   (let ((mbv (make-mbytevector size 0)))
                                     (let loop ((i 0) (j 0))
                                       (define (slash-escape b)
                                         (mbytevector-set! mbv j 92)
