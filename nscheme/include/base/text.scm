...
(define (reader:datum)
  (mlet ((state #f) (frame* '()))
    (define (make-seq shape type len current-len x*) (vector shape type len current-len x*))
    (define (make-empty-seq shape type len) (make-seq shape type len 0 '()))
    (define (seq?               x) (vector? x))
    (define (seq-shape          s) (vector-ref s 0))
    (define (seq-type           s) (vector-ref s 1))
    (define (seq-length         s) (vector-ref s 2))
    (define (seq-current-length s) (vector-ref s 3))
    (define (seq-x*             s) (vector-ref s 4))
    (define (seq-push  s x) (make-seq (seq-shape s) (seq-type s) (seq-length s)
                                      (+ (seq-current-length s) 1) (cons x (seq-x* s))))
    (define (seq-cons* s)   (make-seq (seq-shape s) 'cons* (seq-length s) (seq-current-length s)
                                      (seq-x* s)))
    (define (seq-full? s)   (and (seq-length s) (= (seq-length s) (seq-current-length s))))
    (define (halt x) (set! state x) #f)
    (define (halt/datum datum) (halt (cons 'datum datum)))
    (define (fail loc text desc . x*)
      (halt (cons 'error (make-read-error (if (null? x*) desc (cons desc x*)) loc text))))
      ...
      (lambda (loc text shape type len)
        (let ((top (frame*-top)))
          (cond ((eqv? top 'rb) (fail loc text "too many values following dot"))
                ((and (seq? top) (eqv? (seq-type top) 'bytevector)) (fail loc text "not a byte"))
                ((and (seq? top) (seq-full? top)) (fail-full top loc text))
                (else (frame*-push (make-empty-seq shape type len))))))
      (lambda (loc text shape)
        ...
        (let ((top (frame*-top)))
          (cond ((eqv? top 'rb)
                 (frame*-pop)
                 (let ((top (frame*-top)))
                   (check-shape top (lambda () (let ((x* (seq-x* top)))
                                                 (frame*-pop)
                ...
                ((seq? top)
                 (check-shape
                   top (lambda ()
                         (define (go x*) (let ((x* (reverse x*)))
                                           (frame*-pop)
                                           (push-datum (case (seq-type top)
    ...
    (let ((utf8->n (make-utf8->number
      ...
      (lambda (reader port)
        (mlet ((buf (make-mbytevector 8192 0)) (pos.buf 0) (end.buf 0) (eof? #f) (pos.abs 0))
          (define (buf-span count) (mbytevector->bytevector buf pos.buf count))
              ...
              (seq-alt*
                i (lambda ()
                    (peek i (lambda (c width)
                              (if c
                                  (and (or (not (eqv? c byte:newline)) (reader-newline reader i))
                                       (loop (+ i width) level))
                                  (fail i (buf-span i) "EOF before end of block comment")))))
                (list byte:pipe byte:#) (lambda ()
                                          (let ((i (+ i 2)))
          ...
          (define (Comment:line)
            (let loop ((i 1))
              (peek i (lambda (c width)
                        (if (or (not c) (eqv? c byte:newline)) (comment i) (loop (+ i 1)))))))
          (define (Boolean x)
            (peek 2 (lambda (c w)
...
;; TODO: improve on this sketch
(define (read/port/k port kf keof k) (let ((r (reader:datum)))
                                       (denotate r port)
                                       (let ((st (reader-state r)))
                                         (cond ((eqv? st       'eof)   (keof))
                                               ((eqv? (car st) 'datum) (k  (cdr st)))
                                               ((eqv? (car st) 'error) (kf (cdr st)))
                                               (else (error "incomplete read"))))))
...
#;(let* ((out standard-output-port)
  ...
  (let* ((x->write (lambda (x)
    ...
    (example-write (read/port/k (iport:bytevector text.example) raise error values))
    (oport-write-byte standard-output-port 10)
    (example-write (read/port/k (iport:bytevector text.another-example) raise error values))
    (oport-write-byte standard-output-port 10)
    ;(example-write (read/port/k (iport:bytevector text.failure-example) raise error values))
    ))
