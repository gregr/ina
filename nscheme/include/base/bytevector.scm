...
(define (bytevector-slice bv start count)
  ...
  (let ((len (bytevector-length bv)))
    (let loop-segment ((start 0))
      (let loop-byte ((i start))
        (define (make-segment) (bytevector-slice bv start (- i start)))
