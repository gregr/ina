...
(define (nonpositive-integer?! x) (unless (nonpositive-integer? x)
                                    (error "not a nonpositive integer" x)))
(define (positive-integer?  x) (and (integer? x) (< 0 x)))
(define (positive-integer?! x) (unless (positive-integer? x) (error "not a positive integer" x)))
(define (negative-integer?  x) (and (integer? x) (< x 0)))
(define (negative-integer?! x) (unless (negative-integer? x) (error "not a negative integer" x)))
