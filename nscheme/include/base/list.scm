...
(define (atree-ref*         a* key*)
  (atree-ref*/k a* key* (lambda () (error "atree-ref* with missing key*" key*)) (lambda (v) v)))
(define (atree-ref/k a key* kf k) (atree-ref*/k (list a) key* kf k))
(define (atree-ref/default a key* default) (atree-ref/k a key* (lambda () default) (lambda (v) v)))
(define (atree-ref         a key*)
  (atree-ref/k a key* (lambda () (error "atree-ref with missing key*" key*)) (lambda (v) v)))
(define (atree-set a key* value)
  (atree-replace a (let loop ((key* key*))
                     (if (null? key*) value (list (cons (car key*) (loop (cdr key*))))))))
...
(define (alist-ref         a key)
  (alist-ref/k a key (lambda () (error "alist-ref with missing key" key)) (lambda (v) v)))
...
(define (alist-update/default a key update default)
  (alist-update/k a key update (lambda () (cons (cons key default) a)) (lambda (a) a)))
(define (alist-update a key update)
  (alist-update/k a key update (lambda () (error "alist-update with missing key" key)) (lambda (a) a)))
