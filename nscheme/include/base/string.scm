  ...
  ((define byte:+ 43)
   ...
   (define (decimal->utf8 n radix radix.exponent use-exponent-below use-exponent-above)
     ...
     (let ((radix.exponent (or radix.exponent radix)))
       ...
       (let ((mag (abs n)))
         (cond
           ((and use-exponent-below (< mag use-exponent-below))
            (error "TODO: decimal->utf8" n radix radix.exponent use-exponent-below
                   use-exponent-above))
           ((and use-exponent-above (< use-exponent-above mag))
            (let* ((power             (floor-log mag radix))
                   (power-digit-count (+ (floor-log power radix.exponent) 1))
              ...
              (let-values (((non-repeating-count repeating-count) (rhs-digit-counts part radix)))
                (let* ((part-len      (+ non-repeating-count repeating-count))
                       ...
                       (len           (+ part-len power-digit-count 2 (if (< n 0) 1 0)))
                       (mbv           (make-mbytevector len 0)))
                  (let* ((i (if (< n 0)
                                (begin (mbytevector-u8-set! mbv 0 byte:-) 1)
                                0))
                         (i (begin (set-nat-radix-digits! mbv i 1 whole radix) (+ i 1)))
                         (i (if (< 0 part-len)
                                (begin
                                  (mbytevector-u8-set! mbv i byte:.)
                                  (let* ((i (+ i 1))
                                    ...
                                    (if (< 0 repeating-count)
                                        ...
                                        i)))
                                i))
                         (i (begin (mbytevector-u8-set! mbv i (if (<= radix 10) byte:e byte:p))
                                   (+ i 1))))
                    (set-nat-radix-digits! mbv i power-digit-count power radix.exponent)
                    (mbytevector->bytevector mbv))))))
           (else (error "TODO: decimal->utf8" n radix radix.exponent use-exponent-below
                        use-exponent-above)))))))
  (define number->utf8
