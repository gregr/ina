  ...
  (define utf8->number
    (let ((go (lambda (bv radix)
                ...
                (let* ((len    (bytevector-length bv))
                       (digit  (if (< radix 11) digit10 digit16))
                       (digit* (lambda (i succeed.not-digit succeed.end fail.not-digit fail.end)
                                 (if (< i len)
                                     (let* ((b (bytevector-u8-ref bv i)) (x (digit b)))
                                       (if (< -1 x radix)
                                           (let loop ((i (+ i 1)) (n x))
                                             (if (< i len)
                                                 (let* ((b (bytevector-u8-ref bv i)) (x (digit b)))
