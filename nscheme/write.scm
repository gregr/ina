...
;; quote/abbreviations, padding, packing, numeric formats/radixes
...
(define (write-number n out)
  ...
  (define (pr-frac n)
    (pr ".") (if (= n 0) (pr "0")
               (let loop ((n n)) (when (> n 0) (let ((m (* n 10)))
                                                 (pr-digit (truncate m))
                                                 (loop (- m (truncate m))))))))
  (define (pr-exp n e) (let ((m (truncate n)))
                         (pr-nat m) (pr-frac (- n m)) (pr "e") (pr-int e)))
           ...
           (cond ((>= n #e1e10) (let loop ((n n) (e 0))
                 ...
                 ((< 0 n #e1e-3) (let loop ((n n) (e 0))
                                   (cond ((< n 1) (loop (* n 10) (- e 1)))
                                         (else    (pr-exp n e)))))
                 (else (pr-nat (truncate n)) (pr-frac (- n (truncate n)))))))
        ((integer? n) (pr-int n))
