...
;; optional CPS memoization (i.e., tabling support for left-recursion)
;; fail return
;; peek not/peek
;; one not/one alt* not/alt*, implicit seq* on strings, lists, vectors
;; alt seq app-seq let/seq
;; any */seq +/seq ?/seq

;; TODO: Future DSL-based reader implementation
;(define (digit->nat ch)
...
;(define-grammar*
  ...
  ;(Comment:line  (alt ";" (seq "#!" (not/peek "eof")))
                 ;(*/seq (not/one Linebreak)) Linebreak)
  ;(Comment:block (seq "#|" (*/seq (alt (not/alt* "|#")
                                       ;(seq "|" (not/alt* "|#"))
                                       ;(seq "#" (not/alt* "|#"))
                                       ;(seq (*/seq "#") Comment:block)))
                      ;(*/seq "|") "|#"))
  ;(Comment (alt Comment:datum Comment:line Comment:block))
  ;(Space   (alt WSpace Comment))
  ;(Datum (seq (*/seq Space) (alt (alt Hash
                                      ;(List "(" ")")
                                      ;(List "[" "]")
                                      ;(List "{" "}")
                                      ;(Tag  "'" 'quote)
  ...
  ;(String
    ...
    ;(define Escape-code-point (let/seq ((n (seq "\\" (alt* "uU") Escape-code)))
                                ;(define units (code-point->code-units n))
    ...
    ;(app/seq (lambda (css) (list->string (append* css)))
      ;(seq "\"" (seq0 (*/seq (alt (not/alt* "\\\"") Ecodes Echars)) "\""))))

  ;(Symbol (app/seq (lambda (css) (string->symbol (list->string (append* css))))
