  ...
  (define (String k)
    (let loop ((ch (next)) (acc '()))
      ...
      (define (escape-code-point radix)
        (let cp-loop ((num (list radix (char "#"))))
          (define ch (next))
          (cond ((char=? ch ";")
                 (let* ((cp (rlist->string num)) (n (string->number cp)))
                   (cond ((integer->octets n)
                          => (lambda (bs)
                               (loop (next) (reverse-append bs acc))))
                         (else (k:error (list "invalid code point" cp))))))
                ((not ch) (k:error (list "missing code delimiter" num)))
                (else (cp-loop (cons ch num))))))
      (define (escape ch)
