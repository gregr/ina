...
(test-evaluation
 ...
 '(string-writing
   ...
   (number->string/decimal/repeat 5/1275120) ==> "3.~921199573373486416964677834243051634355982182069138590877721312503e-6")
   ...
   (call/oport:bytes
    (lambda (out)
      (let* ((p (posix-raw-process/k #f #f 'stdout (find-file "echo") '("hello world") panic values))
        ...
        (oport-close (posix-process-in p))
        (let loop ()
          (case-values (iport-read-byte out.p.out)
            (()  (iport-close out.p.out)
                 (oport-write-byte out (+ (posix-process-wait p) 48)))
   ...
   (call/oport:bytes
    (lambda (out)
      (let* ((p (posix-raw-process/k #f #f 'stdout (find-file "cat") '() panic values))
        ...
        (let loop ()
          (case-values (iport-read-byte out.p.out)
            (()  (iport-close out.p.out) (oport-write-byte out (+ (posix-process-wait p) 48)))
            ((b) (oport-write-byte out b) (loop)))))))
   ==>
   #"another example0"
   (let-values (((out current) (oport:bytes&current)))
   ...
   (call/oport:bytes
    (lambda (result)
      (let* ((p1     (posix-raw-process/k #f #f 'stdout (find-file "echo") '("pipe test") panic values))
             ...
             (p2     (posix-raw-process/k fd.in1 #f 'stdout (find-file "cat") '() panic values))
             (in2    (posix-process-out p2)))
        (oport-close (posix-process-in p1))
        (let loop ()
          (case-values (iport-read-byte in2)
            (()  (iport-close in2)
                 (oport-write-byte result (+ (posix-process-wait p1) 48))
