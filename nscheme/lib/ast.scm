...
(define (ast-elaborate ast)
  ...
  (let ev ((env '()) (ast ast))
    (define (param-elaborate bind param/names arg body)
      (define param/strings (param-map name->string param/names))
      (define param/ids (param-map name->id param/names))
      (define nid (let ((i -1)) (lambda () (set! i (+ i 1))
                                  (string-append 'p. (natural->string i)))))
      (define rest (ev (env-extend* env (param-names param/names)
                                    (param-names param/ids)) body))
    ...
    (cond ((? 'var   ) (ast:var (name->id (@ 1))))
          ((? 'set!  ) (astx:let (list 'a.set!) (list (ev env (@ 2)))
                                 (param-elaborate
                                   (lambda (p a rest)
                                     (astx:begin (list (ast:set! p a)) rest))
                                   env (@ 1) (ast:var 'a.set!) (ast:quote #t))))
