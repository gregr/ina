...
(define (list->improper-list d)
  (let ((rd (reverse d))) (reverse-append (cdr rd) (car rd))))
(define (~list->list d)
  (cond ((null? d) '())
...
(define (param*->addr* n*) (map (lambda (n) (and (name? n) (fresh-uid n))) n*))
(define (bpair*?! b*)
...
(define ($lambda ~p?* b?*->body)
  (define p?* (~list->list ~p?*)) (param*?! p?*)
  (define a?* (param*->addr* p?*))
  (define ~a?* (if (improper-list? ~p?*) (list->improper-list a?*) a?*))
  (ast:lambda ~a?* (b?*->body (map cons p?* a?*))))
