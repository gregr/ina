...
(define (expand env form)
  (let loop ((form form)) (define ? #f)
    (cond ((and (set! ? (parse:op? env form)) ?) ?)
...
(define (expand:definition* st form*)
  (foldl (lambda (form st) (define ? #f)
           (cond ((and (set! ? (parse:def? st form)) ?) ?)
              ...
              (let loop ((last #t) (xs xs) (xss xss))
                (and last (if (null? xs) last
                            (loop (apply f (car xs) (map car xss))
                                  (cdr xs) (map cdr xss)))))))
    (ormap (lambda (f xs . xss) (define ? #f)
             (cond ((null? xs) #f)
                   ((and (set! ? (apply f (car xs) (map car xss))) ?) ?)
...
(define parsers:primitive
  '((apply ((length=? 3 form) (ast:apply (ex (@ 1)) (ex (@ 2)))))
    ...
    (cond ((and (length>=? 2 form)
           ...
           (foldr (lambda (c $*)
                    (if (null? (cdr c)) (expand:or2 env (car c) $*)
                      (ast:if (ex (car c)) (expand:body* env (cdr c)) $*)))
