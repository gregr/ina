...
(define parsers:primitive
  '((apply (_ (guard (length=? 3 form)) (ast:apply (ex (@ 1)) (ex (@ 2)))))
    (quote (_ (guard (length=? 2 form)) (ast:quote (@ 1))))
    (if    (_ (guard (length=? 4 form))
              (ast:if (ex (@ 1)) (ex (@ 2)) (ex (@ 3)))))
    (set!  (_ (guard (and (length=? 3 form) (name? (@ 1))))
              (parse:set! env (@ 1) (ex (@ 2)))))
    (error (_ (guard (and (length>=? 1 form) (list? (@. 1))))
              (expand:error env (@. 1))))
    (reset (_ (guard (length>=? 2 form))
              (ast:reset (expand:body* env (@. 1)))))
    (shift (_ (guard (length>=? 3 form))
              (ast:shift ($lambda #f (list (@ 1))
                                  (b?*->expand:body* env (@. 2))))))
    (lambda (_ (guard (length>=? 3 form)) (expand:lambda env (@ 1) (@. 2))))
    (letrec (_ (guard (length>=? 3 form))
               (bpair*?! (@ 1))
               (expand:letrec env (map car (@ 1)) (map cadr (@ 1))
                              (lambda (env) (expand:body* env (@. 2))))))
    (let (_ (guard (and (length>=? 4 form) (name? (@ 1))))
            (bpair*?! (@ 2))
      ...
      (_ (guard (length>=? 3 form))
         (bpair*?! (@ 1))
         (expand:let env (map car (@ 1)) (map cadr (@ 1)) (@. 2))))
    (let* (_ (guard (length>=? 3 form))
             (bpair*?! (@ 1))
    ...
    (begin (_ (guard (length>=? 2 form))
              (define rb* (reverse (map ex (@. 1))))
              ($begin (reverse (cdr rb*)) (car rb*))))
    (cond
      (_ (guard (length>=? 2 form))
         (let loop ((c* (@. 1)))
    ...
    (and (_ (guard (length>=? 1 form)) (expand:and* env (@. 1))))
    (or (_ (guard (length>=? 1 form))
           (foldr (lambda (e r) (expand:or2 env e r)) $false (@. 1))))
    (when (_ (guard (length>=? 3 form))
             (ast:if (ex (@ 1)) (expand:body* env (@. 2)) $true)))
    (unless (_ (guard (length>=? 3 form))
