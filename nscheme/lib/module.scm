((provide premodule premodule-language premodule:parse
 ...
 (require ast:quote ast:var ast:apply* ast:lambda ast:let ast:let* ast:list
          ast-eval rename binding:var env:empty env-extend* env-update*
          @body*))
...
(define (component-require c) (vector-ref c 0))
(define (component-provide c) (vector-ref c 1))
(define (component-ast c)     (vector-ref c 2))
(define (component r p ast)   (vector r p ast))

(define (module-components m) (vector-ref m 0))
(define (module-ast$ m)       (vector-ref m 1))
(define (module-ast m)        ((module-ast$ m)))
(define (module-require m)    (vector-ref m 2))
(define (module-provide m)    (vector-ref m 3))
(define (module-prims m)      (vector-ref m 4))
(define (module-meta m)       (vector-ref m 5))
(define (module$ components ast/components r p prims meta)
  (vector components ast/components r p prims meta))
(define (module r p ast prims meta)
  (module$ (list (component r p ast)) (lambda () ast) r p prims meta))
(define (module:ast->ast m ast->ast)
  (let ((ast (ast->ast (module-ast m)))
        (r (module-require m)) (p (module-provide m)))
    (module$ (list (component r p ast)) (lambda () ast) r p
             (module-prims m) (module-meta m))))
;; TODO: check consistency of duplicate prim names.
(define (module:prims m prims)
  (module$ (module-components m) (module-ast$ m) (module-require m)
           (module-provide m) (append prims (module-prims m)) (module-meta m)))
(define (module:meta m meta)
  (module$ (module-components m) (module-ast$ m) (module-require m)
           (module-provide m) (module-prims m)
...
(define (module:premodule name=>language pm)
  ...
  (define r (append (premodule-require-public pm) pub))
  ...
  (define body (if (premodule-provide-private pm)
                 ...
                 (premodule-body pm)))
  (define ast (ast:lambda private-req (@body* env body)))
  (define p (premodule-provide-public pm))
  (module$ (list (component r p ast)) (lambda () ast) r p '() '()))
(define (module:compose become? ma mb)
  ...
  (define cs:a   (module-components ma))
  (define req:a  (module-require ma))
  (define pro:a  (module-provide ma))
  (define prim:a (module-prims ma))
  (define meta:a (module-meta ma))
  (define cs:b   (module-components mb))
  (define req:b  (module-require mb))
  (define pro:b  (module-provide mb))
  (define prim:b (module-prims mb))
  (define meta:b (module-meta mb))
  (define cs   (append cs:a cs:b))
  (define req  (napp (filter-not (lambda (n) (member n pro:a)) req:b) req:a))
  ;; TODO: check for duplicates.
  (define pro  (if become? pro:b (append pro:b pro:a)))
  (unless (list? pro:b) (error (list 'nope: pro:b)))
  (define prim (append prim:b prim:a))
  (define meta (env-update* meta:a meta:b))
  (define (ast/components)
    (define $m* (map (lambda (c) (rename 'module)) cs))
    (define result*
      (map (lambda (c $m)
             (ast:apply* (ast:var $m) (map ast:var (component-require c))))
           cs $m*))
    (ast:let $m* (map component-ast cs)
             (ast:lambda req (ast:let* (map component-provide cs) result*
                                       (apply ast:list (map ast:var pro))))))
  (if become? (let ((ast (ast/components)))
                (module$ (list (component req pro ast))
                         (lambda () ast) req pro prim meta))
    (module$ cs ast/components req pro prim meta)))
