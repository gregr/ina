...
(define (E-stage E cenv)
  (let loop.full ((E E) (cenv cenv))
    ...
    (match E
      (`#(E:annotated ,_ ,E)       (loop E))
      (`#(E:quote     ,value)      (lambda (renv) value))
      (`#(E:ref       ,address)    (cenv-ref cenv address))
      (`#(E:if        ,c ,t ,f)    (let ((s.c (loop c)) (s.t (loop t)) (s.f (loop f)))
                                     (lambda (renv) (if (s.c renv) (s.t renv) (s.f renv)))))
      (`#(E:call      ,proc ,arg*) (let ((s.proc (loop proc)) (s*.arg (map loop arg*)))
                                     (lambda (renv) (apply (s.proc renv)
                                                           (map (lambda (s) (s renv)) s*.arg)))))
      (`#(E:case-lambda ,param* ,body*)
        (let ((k (foldr (lambda (param body k)
                          (let* ((cenv   (cenv-extend cenv param))
                                 (s.body (loop.full body cenv))
                            ...
                            (lambda (renv count args)
                              (let ((renv-extend (make-renv-extend arity count)))
                                (if renv-extend
                                    ...
                                    (k renv count args))))))
                        (lambda (renv count args)
                          (error "arity mismatch" 'given count 'expected param*))
                        param* body*)))
          (lambda (renv)
            (lambda args (k renv (length args) args)))))
      (`#(E:letrec ,param ,rhs* ,body)
        (let* ((arity       (param-arity param))
               (renv-extend (make-renv-extend arity (length param)))
               (s.body      (loop.full body (cenv-extend cenv param)))
               (cenv        (cenv-extend-rec cenv param))
               (s*.arg      (map (lambda (rhs) (loop.full rhs cenv)) rhs*)))
          (lambda (renv)
...
(define (E-pretty E)
  (let loop ((E E))
    (match E
      (`#(E:annotated   ,_ ,E)        (loop E))
      (`#(E:quote       ,value)       `(quote ,value))
      (`#(E:ref         ,address)     `(ref ,address))
      (`#(E:if          ,c ,t ,f)     `(if ,(loop c) ,(loop t) ,(loop f)))
      (`#(E:call        ,proc ,arg*)  `(call ,(loop proc) . ,(map loop arg*)))
      (`#(E:case-lambda ,p* ,body*)   `(case-lambda . ,(map list p* (map loop body*))))
      (`#(E:letrec ,lhs* ,rhs* ,body) `(letrec ,(map list lhs* (map loop rhs*)) ,(loop body)))
      (`#(E:prim ,name)               `(prim ,name)))))
