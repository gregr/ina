  ...
  (for-each
   (lambda (test-part*)
     (when (< 0 verbosity)
       (rkt:newline)
       (display-border)
       (rkt:pretty-write (car test-part*))
       (display-border))
     (let loop ((test-part* (cdr test-part*)))
       (unless (null? test-part*)
         ...
         (let ((test-case (list (car test-part*) (cadr test-part*) (caddr test-part*))))
           ...
           (when (< 2 verbosity) (rkt:pretty-write test-case))
           (let* ((stx.expr (car test-case))
             ...
             (cond
               ...
               (else (set! test-failures (cons (cons actual test-case) test-failures))
                     (unless (< 0 verbosity) (rkt:newline) (rkt:pretty-write test-case))
...
(test-evaluation
 ...
 '(cmdline
   (current-host-argument*) ==> #(#"bootstrap/test.rkt"))
 ...
 '(host-processes
   (call/oport:bytevector
    (lambda (out)
      (let* ((p (host-raw-process/k #f #f 'stdout (find-file "echo") '("hello world") panic values))
   ...
   (let-values (((out current) (oport:bytevector&current)))
     (let ((p (host-process empty-iport out 'stdout (find-file "echo") '("hello world"))))
       (values (host-process-wait p) (current))))
   ==>
   (values 0 #"hello world\n")
   (call/oport:bytevector
    (lambda (out)
      (let* ((p (host-raw-process/k #f #f 'stdout (find-file "cat") '() panic values))
   ...
   (let-values (((out current) (oport:bytevector&current)))
     (let* ((in (iport:bytevector #"another example"))
            (p  (host-process in out 'stdout (find-file "cat") '())))
       (values (host-process-wait p) (current))))
   ==>
   (values 0 #"another example")
   (call/oport:bytevector
    (lambda (result)
      (let* ((p1     (host-raw-process/k #f #f 'stdout (find-file "echo") '("pipe test") panic values))
             (in1    (host-process-out p1))
             (fd.in1 (cdr (assoc 'file-descriptor (port-describe in1))))
             (p2     (host-raw-process/k fd.in1 #f 'stdout (find-file "cat") '() panic values))
   ...
   (let-values (((result current) (oport:bytevector&current)))
     (let* ((p1 (host-process empty-iport #f 'stdout (find-file "echo") '("pipe test")))
            (p2 (host-process (host-process-out p1) result 'stdout (find-file "cat") '())))
