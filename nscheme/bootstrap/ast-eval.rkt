...
(define (ast-stage ast cenv)
  (let loop.full ((ast ast) (cenv cenv))
    ...
    (match ast
      ...
      (`#(E:call  ,_ ,a.proc ,a*.arg) (let ((s.proc (loop a.proc)) (s*.arg (map loop a*.arg)))
...
(define (ast-pretty ast)
  (let loop ((ast ast))
    (match ast
      ...
      (`#(E:call  ,_ ,proc ,arg*)        `(call ,(loop proc) . ,(map loop arg*)))
      (`#(E:case-lambda ,_ ,cc*)         `(case-lambda
                                            . ,(map (lambda (cc)
                                                      (list (case-lambda-clause-param cc)
                                                            (loop (case-lambda-clause-body cc))))
                                                    cc*)))
      (`#(E:letrec ,_ ,lhs* ,rhs* ,body) `(letrec ,(map (lambda (lhs rhs) (list lhs (loop rhs)))
                                                        lhs* rhs*)
                                            ,(loop body)))
      (`#(E:prim ,_ ,name)               `(prim ,name)))))
