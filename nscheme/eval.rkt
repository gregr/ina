...
(define (@lambda env param body)
  (let ((cenv (alist-remove* env (param-names param))))
    (lambda (a) (eval (env-extend*/var cenv (param-bind param a)) body))))
...
(define (@let env b* body)
  (@apply (@lambda env (map car b*) body) (eval* env (map cadr b*))))
(define (@let/name env name b* body)
  (define p (plift (lambda (env) (@lambda env (map car b*) body))))
