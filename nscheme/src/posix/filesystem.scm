...
(splicing-let ((byte:/ (bytevector-ref #"/" 0)))
  ...
  (define (path-directory path)
    (let* ((path (path->bytevector path))
           (len  (bytevector-length path)))
      (if (< 0 len)
          (let* ((base-end   (if (eqv? (bytevector-ref path (- len 1)) byte:/) (- len 1) len))
                 (base-start ((bytevector-index-end/byte byte:/) path base-end)))
            (if (< 0 base-start)
                (bytevector-copy path 0 (- base-start 1))
                #"."))
          #".")))
  (define (path-base path)
    (let* ((path (path->bytevector path))
      ...
      (if (< 0 len)
          (let* ((base-end   (if (eqv? (bytevector-ref path (- len 1)) byte:/) (- len 1) len))
                 (base-start ((bytevector-index-end/byte byte:/) path base-end)))
            (bytevector-copy path base-start (- base-end base-start)))
          #"")))
