...
(begin-meta
  ...
  (splicing-local
    ((define SP:any                  (vector 'SP:any))
     ...
     (define ((SP-compile x stx) env $x $fail succeed)
       (let loop-0 ((x x) ($x $x) (succeed succeed))
         (SP-case
           x (lambda (SP) (mistake 'SP-compile "not an SP" SP))
           SP:any?          (lambda ()      (succeed))
           ...
           SP:var?          (lambda (id)    (let ((addr (make-address (syntax-unwrap id) id)))
                                              (env-vocabulary-bind! env id vocab.syntax-template (cons addr 0))  ; detects duplicates
                                              (E:let (list addr) (list $x) (succeed))))
           SP:cons?         (lambda (a b)   ($let1 'unwrap ($syntax-unwrap $x)
                                                   (lambda ($x)
                                                     ($if ($pair? $x)
                                                          ($let '(car.x cdr.x) (list ($car $x) ($cdr $x))
                                                                (lambda ($car.x $cdr.x)
                                                                  (loop-0 a $car.x (lambda () (loop-0 b $cdr.x succeed)))))
                                                          $fail))))
           SP:list->vector? (lambda (p)     ($let1 'unwrap ($syntax-unwrap $x)
                                                   (lambda ($x)
                                                     ($if ($vector? $x)
                                                          ($let 'x ($vector->list $x) (lambda ($x) (loop-0 p $x succeed)))
                                                          $fail))))
           SP:map?          (lambda (p stx) (mistake 'SP-compile "TODO: SP:map" x)))))
