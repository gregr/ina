...
(begin-meta
  ...
  (splicing-local
    ((define (ST:quote        stx)        (vector 'ST:quote        stx))
     ...
     (define (ST:ref          addr level) (vector 'ST:ref          addr level))
     ...
     (splicing-local
       ...
       (define (ST:ref?          x) (ST-tagged? x 3 'ST:ref))
     ...
     (define (ST-compile x)
       (let-values
         (((E addr&level=>addr addr=>E)
           (let loop ((x x) (addr&level=>addr '()) (addr=>E '()))
             ...
             (ST-case
               ...
               ST:ref?          (lambda (addr level)
                                  (if (< 0 level)
                                      (let ((a&l (cons addr level)))
                                        (alist-ref/k addr&level=>addr a&l
                                                     (lambda ()
                                                       (let ((addr (make-address (address-name addr) (address-note addr))))
         ...
         (unless (null? addr&level=>addr) (raise-parse-error "not enough ellipses" (caaar addr&level=>addr)))
    ...
    (define (parse-syntax-template env stx)
      (let loop ((stx stx))
        (let ((a&l (and (identifier? stx) (env-vocabulary-ref env stx vocab.syntax-template))))
          (if a&l
              (let ((addr (car a&l)) (level (cdr a&l))) (ST:ref addr level))
