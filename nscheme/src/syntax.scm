...
(splicing-local
  ((define rtd.mark    (make-rtd 'mark 1 #f))
   ...
   (define rtd.annotated    (make-rtd 'syntax:annotated 2 #f))
   ...
   (define rtd.marked    (make-rtd 'syntax:marked 2 #f))
         ...
         (define (self method)
           (case method
             ...
             ((read-only)  (if read-only? self (let ((env (env-disjoin env.mark m env.no-mark)))
                                                 ...
                                                 env)))
             ((frozen?)    #f)
             ((writable)   (and (not read-only?) (or (and (env.no-mark 'writable) self)
                                                     (begin (set! read-only? #t) #f))))
             ((describe)   (env.no-mark 'describe))
             (else         (mistake "invalid environment operation" method))))
         self)))
    (define (syntax-transcribe stx op env.op env.use)
      (let* ((env.d.op (and (env.use 'writable) (make-env)))
             (env.op   (if env.d.op (env-read-and-write (env-conjoin env.d.op env.op) env.d.op) env.op))
             (m        (fresh-mark))
