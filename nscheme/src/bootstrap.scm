...
(define (make-library=>env load-library)
  ...
  (splicing-let ((env.deps (env-conjoin* env.base env.syntax env.compiler)))
    (define env.parser (load-library env.deps 'parser))
    (define env.medium (env-conjoin* env.deps env.parser env.meta)))
  (define env.posix     (load-library env.base 'posix))
  (splicing-let ((env.deps (env-conjoin* env.medium env.posix)))
    (define env.bootstrap (load-library env.deps 'bootstrap))
    (define env.large     (env-conjoin* env.deps env.bootstrap)))
