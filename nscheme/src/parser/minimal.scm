...
(define (parse-body env stx.body)
  (let ((env.d (make-env)))
    (D->E ($d:end/expression
            (parse-begin-definition* env.d (env-conjoin env.d env) (syntax->list stx.body))
            stx.body))))
...
(define (parse-mdefine env.d env id.lhs stx.rhs)
  (let ((env.local (make-env)) (env (env-read-only env)))
    (env-introduce-boxed! env.d id.lhs (let ((env.local (env-read-only env.local)))
