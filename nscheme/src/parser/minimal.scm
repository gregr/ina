...
(define (parse-mdefine env id.lhs stx.rhs)
  ...
  (let* ((env.local (make-env))
         (D         ($d:define env.local id.lhs (let ((env (env-read-only env)))
                                                  (lambda () ($box (parse-expression env stx.rhs)))))))
    (env-read-only! env.local)
    (env-bind-boxed! env id.lhs (lambda () (parse-expression env.local id.lhs)))
    D))
             ...
             (cond
               ...
               (else (let* ((c (car c*)) (c* (cdr c*)) (e* (syntax->list c)))
                       ...
                       (let ((e.data (car e*)) (e* (cdr e*)))
                         (when (null? e*) (raise-parse-error "empty clause body" c))
                         (let (($rhs (if (eqv? (keyword (car e*)) '=>)
                                         (if (and (pair? (cdr e*)) (null? (cddr e*)))
                                             ($call (parse-expression env (cadr e*)) $x)
                                             (raise-parse-error "=> is not followed by one procedure" c))
                                         (parse-body env e*))))
                           (if (eqv? (keyword e.data) 'else)
                               (if (null? c*) $rhs (raise-parse-error "else clause is not last" c))
                               ($if (apply $or (map (lambda (d) ($literal-equal? d $x))
                                                    (syntax->datum e.data)))
                                    $rhs
                                    (loop c*))))))))))))
              ...
              (lambda (env.unboxed)
                (let ((env.boxed (make-env)))
                  (for-each (lambda (id)
                            ...
                            param*)
                  (env-read-only! env.boxed)
                  (parse-body (env-conjoin env.boxed env) stx*.body))))))
...
(define ((make-parse-quasi vocabulary-name tag.qq tag.unq tag.unq-splicing tag.ellipsis
  ...
  (let loop ((stx stx) (level 0) (quote-ellipsis? (not tag.ellipsis)) (k (lambda (q) q)) (k-splice #f) (k-quote (lambda () ($quote stx))))
    ...
    (let ((form (syntax-unwrap stx)))
      (cond ((pair? form)
             (let ((tag (keyword (car form))))
               (define (k-embed q) (k ($cons ($quote (car form)) q)))
               (if tag
                   ...
                   (let ((a (car form)) (b (cdr form)))
                     (let ((form.b (and (not quote-ellipsis?) (syntax-unwrap b))))
                       (if (and (pair? form.b) (eqv? (keyword (car form.b)) tag.ellipsis))
                           (let loop-ellipsis ((b (cdr form.b)) (stx*.ellipsis (list (car form.b))))
                             (let ((form.b (syntax-unwrap b)))
                               (if (and (pair? form.b) (eqv? (keyword (car form.b)) tag.ellipsis))
                                   (loop-ellipsis (cdr form.b) (cons (car form.b) stx*.ellipsis))
                                   (let ((finish
                                           (lambda ($Q q.a)
                                             (let loop-map ((q.a q.a) (stx*.ellipsis (reverse stx*.ellipsis)))
                                               (let ((q.a ($map q.a (car stx*.ellipsis))) (stx*.ellipsis (cdr stx*.ellipsis)))
                                                 (if (null? stx*.ellipsis)
                                                     (k ($Q q.a (loop b level #f (lambda (q) q) #f (lambda () ($quote b)))))
                                                     (loop-map q.a stx*.ellipsis)))))))
                                     (loop a level #f
