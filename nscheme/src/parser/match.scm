(define vocab.match-pattern 'match-pattern)
(define vocab.match-guard   'match-guard)
...
(define (parse-pattern-cons* env stx . stx*)
  (define keyword ((keyword/env/vocabulary vocab.match-pattern) env))
  ...
  (define ((^default) env stx)
    (let* ((x (syntax-unwrap stx)))
      (cond ((symbol?  x) ($p:var stx))
            ((literal? x) ($p:quote x))
            (else         (raise-parse-error (list vocab.match-pattern "invalid syntax") stx)))))
  ($p:source ((vocabulary-parser vocab.match-pattern ^default) env stx) stx))

(define (parse-pattern-quasiquote env stx.qq)
  (define pattern-keyword ((keyword/env/vocabulary vocab.match-pattern) env))
                                     ...
                                     (lambda ($fail)
                                       ($let1 'succeed
                                              ($lambda/env
                                                env id*.P
                                                (lambda (env)
                                                  (define keyword ((keyword/env/vocabulary vocab.match-guard) env))
...
(define (env-conjoin/match env)
  (let* ((env (let ((env.scope (make-env)))
                (env-read-and-write (env-conjoin env.scope env) env.scope)))
         (b*.match-guard
           '((guard . guard)))
         (b*.expr
         ...
         (b*.match-pattern.extend
           (list (cons 'quote      (operator-parser parse-pattern-quote      1 1))
                 ...
                 (cons 'or         (operator-parser parse-pattern-or         0 #f))
                 (cons 'not        (operator-parser parse-pattern-not        1 1))
                 (cons 'cons       (operator-parser parse-pattern-cons       2 2))
                 (cons 'vector     (operator-parser parse-pattern-vector     0 #f))
                 (cons 'cons*      (operator-parser parse-pattern-cons*      1 #f))
                 (cons 'list       (operator-parser parse-pattern-list       0 #f)))))
    (define (vocab-bind*! vocab id=>op)
      (alist-for-each id=>op (lambda (id op) (env-vocabulary-bind! env id vocab op))))
    (vocab-bind*! vocab.match-guard b*.match-guard)
    (vocab-bind*! vocab.expression  b*.expr)
    (vocab-bind*! vocab.match-pattern b*.match-pattern)
    (alist-for-each b*.match-pattern.extend
                    (lambda (id op) (when (env-ref env id)
                                      (env-vocabulary-add! env id vocab.match-pattern op))))
    (env-freeze env)))
