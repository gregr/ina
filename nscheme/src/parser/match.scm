...
(define (env-conjoin/match env)
  (let* ((env (let ((env.scope (make-env)))
    ...
    (for-each (lambda (id) (env-vocabulary-bind! env id vocab.expression-auxiliary id))
              b*.expr-aux)
    (for-each (lambda (id) (env-vocabulary-bind! env id vocab.pattern-auxiliary id))
              b*.pattern-aux)
    (for-each (lambda (id op) (env-vocabulary-bind! env id vocab.expression op))
              (map car b*.expr) (map cdr b*.expr))
    (for-each (lambda (id op) (env-vocabulary-bind! env id vocab.pattern op))
              (map car b*.match-pattern) (map cdr b*.match-pattern))
    (for-each (lambda (id op) (when (env-ref env id) (env-vocabulary-add! env id vocab.pattern op)))
