...
(define ((parse-match/parse-pattern parse-pattern) env stx.e . stx*.clause*)
  ($let1 'x (parse-expression env stx.e)
         (lambda ($x)
           ($let1 'fail ($thunk ($mistake ($quote "no matching pattern") $x))
                  (lambda ($fail)
...
(define (env-conjoin/match env)
  (let ((env (let ((env.scope (make-env)))
    ...
    (for-each (lambda (id) (env-vocabulary-bind! env id vocab.match-guard id))
              '(guard))
    (alist-for-each
    ...
    (alist-for-each
      ...
      (lambda (id op) (env-vocabulary-bind! env id vocab.match-pattern op)))
    (alist-for-each
