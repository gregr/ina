...
;;              | (set! Location Label)
;;              | (jump-if (Compare-op Location S32) Label)
;;              | (jump-if (Compare-op Location Register) Label)
;;              | (jump Label)
;;              | (jump Location)
;;              | Label
  ...
  (define (LLL-validate-x86-64 P)
      ...
      (and (pair? rhs) (or (binop->op (car rhs)) (mistake "invalid binary operator" rhs))
           ...
           (apply (case-lambda
                    ((a b)
                     ...
                     (or (Register? b) (S32? b)
                         (and (Memory? b) (or (eqv? (mloc-width b) 8)
    ...
    (define (Comparison? x)
      (and (pair? x) (or (cmpop->jcc (car x)) (mistake "invalid comparison operator" x))
           ...
           (apply (case-lambda
                    ((a b) (or (Register? a)
                               (and (Memory? a) (or (eqv? (mloc-width a) 8)
            ...
            (case (car S)
              ((set!) (lambda (lhs rhs)
                        (emit (cond ((Label? rhs) (Instruction "leaq" `#(mloc 8 rip ,rhs 0) lhs))
                                    ((pair? rhs) (apply (lambda (a b) (Binary-op (car rhs) a b))
                                                        (cdr rhs)))
                                    (else (Assign lhs rhs))))))
              ((jump-if) (lambda (cmp label)
                           (let* ((cmpop (car cmp))
