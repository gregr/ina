...
(splicing-local
  ...
  (define (LLL-emit-x86-64-at&t P)
          ...
          ((8) (case wr
                 ((8) (cond ((and (Register? lhs) (eqv? rhs 0)) (Assign-0 lhs))
                            ...
                            ((and (Register? lhs) (integer? rhs)
                             ...
                             (Instruction "movabsq" rhs lhs))
                            (else (Instruction "movq" rhs lhs))))
    ...
    (define (Assign-0 reg) (clear-cmp!) (set! carry? 0) (let ((w4 (register/width reg 4)))
    ...
    (define (Compare! op a b)
      (define (Test op a b) (set! carry? 0) (Instruction op b a))
      (case op
        ((and nand) (if (equal? a b)
                    ...
                    (cond ((and (U32? b) (Register? a))
                           (if (U8? b)  ; NOTE: we skip 16-bit to avoid 66h LCP stalls
                               (Test "testb" (register/width a 1) b)
                               (Test "testl" (register/width a 4) b)))
                          (else (Test "testq" a b))))
        (else (set-cmp! (cons a b) 'cmp)
              (if (and (eqv? b 0) (Register? a))
                  (Test "testq" a a)
                  (begin (set! carry? #f) (Instruction "cmpq" b a))))))
    ...
    (define (Binary-op op a b)
      (let ((iop (binop->op op)))
        (and iop (let ((simple (lambda () (set! carry? #f) (Instruction iop b a)))
                   ...
                   (case op
                     ...
                     ((+/carry -/carry) (case b
                                          ((0) (if (eqv? carry? 0)
                                                   (No-op)
                                                   (begin (set! carry? 0) (Instruction "clc"))))
                                          (else (simple))))
    ...
    (define (Negate x) (set! carry? #f) (Instruction "negq" x))
    (define (CAS loc new) (clear-cmp!) (set! carry? #f) (Instruction "lock cmpxchgq" new loc))
    (let loop ((S P))
      (if (Label? S)
          (begin (clear-cmp!) (set! carry? #f) (emit (string-append S ":\n")))
          (apply
            (case (car S)
              ((set!) (lambda (lhs rhs)
              ...
              ((set2!) (lambda (l1 l2 rhs)
                         (case (car rhs)
                           ((u128*) (apply (lambda (a b) (Instruction "mulq" b)) (cdr rhs))
                                    (set! carry? #f))
