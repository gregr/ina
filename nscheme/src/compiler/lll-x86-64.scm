...
(splicing-local
  ...
  (define (LLL-emit-x86-64-at&t P)
    ...
    (define (Jump op rand) (string-append " " op (if (Label? rand) " " " *") (Operand rand) "\n"))
    ...
    (define (Assign-cc lhs cc) (let ((w1 (register/width lhs 1)))
                                 (string-append (Instruction (string-append "set" cc) w1)
                                                (Instruction "movzbl" w1 (register/width lhs 4)))))
    (define (Cmp-op lhs op a b) (string-append (Instruction (cmpop->test op) b a)
                                               (Assign-cc lhs (cmpop->cc op))))
      ...
      (if (Label? S)
          ...
          (apply
            (case (car S)
              ...
              ((jump-if) (lambda (x l)
                           (let ((op (car x)))
                             (cond ((eqv? op 'cc) (emit (Jump-cc (flagcc (cadr x)) l)))
                                   (else (emit (Instruction (cmpop->test op) (caddr x) (cadr x)))
                                         (emit (Jump-cc (cmpop->cc op) l)))))))
              ((jump) (lambda (x) (emit (Jump "jmp" x))))
              ((call) (lambda (x) (emit (Jump "call" x))))
              ((begin) (lambda S* (for-each loop S*)))
              (else (mistake "not a Statement" S)))
            (cdr S))))))
