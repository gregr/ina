...
(splicing-local
  ...
  (define (LLL-emit-x86-64-at&t P)
    ...
    (define (Assign-cc lhs cc) (let ((w1 (register/width lhs 1)))
                                 (Instruction (string-append "set" cc) w1)
    ...
    (define (Compare op a b)
      (unless (and (equal? current-cmp (cons a b)) (or (not shift-cmp?) (shift-cc op)))
        (set! current-cmp (cons a b)) (set! shift-cmp? #f)
        (if (and (eqv? b 0) (Register? a))
            (Instruction "testq" a a)
            (Instruction (cmpop->test op) b a))))
    (define (Cmp-op lhs op a b) (Compare op a b) (Assign-cc lhs (choose-cc op)))
    (define (Binary-op op a b)
      (let ((op (binop->op op)))
        (and op (begin (case op
                       ...
                       (Instruction op (if (and (eqv? b 'rcx) (Shift? op)) 'cl b) a)
                       #t))))
