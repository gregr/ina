...
(splicing-local
  ...
  (define (LLL-emit-x86-64-at&t P)
    ...
    (define (Compare op a b) (Instruction (cmpop->test op) b a))
    (define (Cmp-op lhs op a b) (Compare op a b) (Assign-cc lhs (cmpop->cc op)))
    (define (Binary-op op a b)
      (let ((op (binop->op op)))
        (and op (begin (Instruction op (if (and (eqv? b 'rcx) (Shift? op)) 'cl b) a)
                       #t))))
              ...
              ((jump-if) (lambda (x l) (let ((op (car x)))
                                         (cond ((eqv? op 'cc) (Jump-cc (flagcc (cadr x)) l))
                                               (else (Compare op (cadr x) (caddr x))
                                                     (Jump-cc (cmpop->cc op) l))))))
              ((jump) (lambda (x) (Jump "jmp" x)))
              ((call) (lambda (x) (Jump "call" x)))
              ((begin) (lambda S* (for-each loop S*)))
              (else (mistake "not a Statement" S)))
            (cdr S))))))
