...
(define (E:quote       v)              (vector 'quote       #f v))
(define (E:ref         address)        (vector 'ref         #f address))
(define (E:if          c t f)          (vector 'if          #f c t f))
(define (E:begin       e^ e)           (vector 'begin       #f e^ e))
(define (E:call        rator rand*)    (vector 'call        #f rator rand*))
...
(splicing-local
  ...
  (define (E:quote?       E) (E-tagged? E 3 'quote))
  (define (E:ref?         E) (E-tagged? E 3 'ref))
  (define (E:begin?       E) (E-tagged? E 4 'begin))
  (define (E:call?        E) (E-tagged? E 4 'call))
  (define (E:case-lambda? E) (E-tagged? E 4 'case-lambda))
  (define (E:if?          E) (E-tagged? E 5 'if))
  (define (E:letrec?      E) (E-tagged? E 5 'letrec)))
  ...
  (let loop ((E E))
    (E-case
      ...
      E:case-lambda? (lambda (param*~* body*)
                       (cons 'case-lambda
                             (map (lambda (p*~ body) (list (improper-list-map address-pretty p*~)
                                                           (loop body)))
                                  param*~* body*)))
      E:letrec?      (lambda (lhs* rhs* body)
                       (list 'letrec (map (lambda (lhs rhs) (list (address-pretty lhs) (loop rhs)))
                                          lhs* rhs*)
                             (loop body))))))
       ...
       (E-case
         ...
         E:begin?       (lambda (e^ e)  (let ((e^ (tree-map loop e^)) (e (loop e)))
                                          (lambda (renv)
                                            ...
                                            (e renv))))
         E:call?        (lambda (rator rand*)
                          (let ((rator (loop rator)) (rand* (map loop rand*)))
                                         ...
                                         (lambda (renv count args)
                                           (let ((renv-extend (make-renv-extend arity count)))
                                             (if renv-extend
                                                 ...
                                                 (k renv count args))))))
                                     (lambda (renv count args)
                                       (mistake 'E-eval "arity mismatch"
                                                'given count 'expected param*~*))
                                     param*~* body*)))
         ...
         E:letrec?      (lambda (param rhs* body)
                          (let* ((arity       (param-arity param))
                            ...
                            (lambda (renv)
                              (let ((loc* (map (lambda (_) (box #t)) arg*)))
