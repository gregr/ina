...
(define (describe x)
  (cond ((record/representer? x) (let ((representer (record-representer x)))
                                   (if (procedure? representer) (representer x) representer)))
        ...
        ((evt?                x) 'evt)
        (else                    #f)))
(define (make-record-type name mutable-field?* representer)
  (let* ((prop* (list (cons prop:sealed #t) (cons prop:representer representer)))
         (field-count (vector-length mutable-field?*))
         (immutable* (let loop ((i 0)) (if (< i field-count)
                                           (if (vector-ref mutable-field?* i)
                                               (loop (+ i 1))
                                               (cons i (loop (+ i 1))))
                                           '()))))
