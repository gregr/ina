(splicing-let
  ((name=>prim
     (aquote
       ...
       current-platform)))
  (define addr=>id (map (lambda (n) (cons (make-address n #f) n)) (map car name=>prim)))
  (define prim=>addr (map cons (map cdr name=>prim) (map car addr=>id))))
...
(define (E-replace-primitive E prim=>addr)
