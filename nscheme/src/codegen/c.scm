            ...
            (apply
              (case (car x)
                ...
                ((call) (lambda (rator . rand*)
                          (list (c:postfix rator) "(" (comma* c:expr rand*) ")")))
    ...
    (define (aggregate tag y*)
      ...
      (list (c:expr tag) " "
            (if (symbol? (car y*))
                ...
                (body y*))))
    (define (make-%cond c c*)
      (when (eqv? (car c) 'else) (mistake "%cond cannot begin with else" x))
      (define (make-c c)
        (list (if (eqv? (car c) 'else) "se " (list "if " (c:expr (car c))))
              (map (lambda (x) (list nl (c:stmt/indent indent x))) (cdr c))))
      (list "#" (make-c c) (map (lambda (c) (list nl "#el" (make-c c))) c*)
            nl "#endif"))
    (if (pair? x)
        (apply
          (case (car x)
            ((fn inline-fn) (lambda (rtype name param* . body*)
                              (append (if (eqv? (car x) 'inline) '("inline ") '())
                                      (list (c:expr rtype) " " name "(" (comma* c:expr param*) ") {"
                                            (map stmt body*) nl "}"))))
            ((return break continue goto) => (lambda (tag) (case-lambda
                                                             (() (list tag ";"))
                                                             ((x) (list tag " " (c:expr x) ";")))))
            ...
            ((%include) (lambda (p . p*)
                          (let loop ((p p) (p* p*))
                            (let ((p (list "#include "
                                           (if (symbol? p) (list "<" p ">") (list "\"" p "\"")))))
                              (if (null? p*) p (list p nl (loop (car p*) (cdr p*))))))))
            ((%define) (lambda (lhs rhs)
                         (list "#define "
                               (if (pair? lhs)
                                   (list (c:expr (car lhs)) "(" (comma* c:expr (cdr lhs)) ")")
                                   (c:expr lhs))
                               " " rhs)))
            ((%undef)  (lambda (name) (list "#undef " (c:expr name))))
            ((%cond)   (lambda (c . c*)   (make-%cond c c*)))
            ((%when)   (lambda (c . body) (make-%cond (cons c body) '())))
            ((%unless) (lambda (c . body) (make-%cond (cons (list 'not c) body) '())))
            ((%call)   (lambda (rator . rand*)
                         (list (c:postfix rator) "(" (comma* c:expr rand*) ")")))
            ((%error %warning)
             => (lambda (tag) (lambda (msg . msg*)
                                (let ((tag (if (eqv? tag '%error) "#error " "#warning ")))
                                  (list tag msg (map (lambda (msg) (list nl tag msg)) msg*))))))
            ((%line) (case-lambda
                       ((number)          (list "#line " (c:expr number)))
                       ((number filename) (list "#line " (c:expr number) " \"" filename "\""))))
            ((V)  (lambda (s) s))
