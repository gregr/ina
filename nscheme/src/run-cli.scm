...
(define (compiler-output*-add! target path)
  (let ((target (bytes->symbol target)))
  ...
  (map (lambda (source)
         (case (car source)
           ...
           ((text)  (let ((text (cadr source)))
                      ...
                      (cons #f ((read*-syntax-annotated/source 'cli) (iport:bytes text)))))
  ...
  (map (lambda (source)
         (case (car source)
           ((file)  (let ((path (cadr source)))
                      ...
                      (cons path (file->bytes path))))
           ((text)  (let ((text (cadr source)))
                      (verbose-write `(read-text ,text))
                      (cons 'cli text)))
           ((stdin) (cons 'stdin (iport->bytes (current-input-port))))
           (else (mistake "unexpected definition source" source))))
       source*))
