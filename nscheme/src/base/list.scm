...
(define ((rem/= =?) y x*) (remp (lambda (x) (=? x y)) x*))
(define remv   (rem/= eqv?))
(define remove (rem/= equal?))

(define (aremp ? k=>x) (remp (lambda (kx) (? (car kx))) k=>x))
(define ((arem/= =?) y x*) (aremp (lambda (x) (=? x y)) x*))
(define aremv   (arem/= eqv?))
(define aremove (arem/= equal?))

(define (premp ? k=>x)
  (let loop ((k=>x k=>x))
    (cond ((null? k=>x)   '())
          ((? (car k=>x)) (cddr k=>x))
          (else           (cons (car k=>x) (cons (cadr k=>x) (loop (cddr k=>x))))))))
(define ((prem/= =?) y x*) (premp (lambda (x) (=? x y)) x*))
(define premv   (prem/= eqv?))
(define premove (prem/= equal?))

(define (assp ? alist)
  (let loop ((alist alist))
    (and (not (null? alist))
         (let ((kv (car alist)))
           (if (? (car kv)) kv (loop (cdr alist)))))))
...
(define (filter ? x*)
  (let loop ((x* x*))
    (cond ((null? x*)   '())
          ...
          (else         (loop (cdr x*))))))
(define (afilter ? k=>x) (filter (lambda (kx) (? (car kx))) k=>x))
(define (pfilter ? k=>x)
  (let loop ((k=>x k=>x))
    (cond ((null? k=>x)   '())
          ((? (car k=>x)) (cons (car k=>x) (cons (cadr k=>x) (loop (cddr k=>x)))))
          (else           (loop (cddr k=>x))))))
