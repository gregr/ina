...
(define (printer-fill p width byte attr)
  (mlet ((size 0))
    (make-printer
      ...
      (lambda ()
        (when (< size width) (printer-print p (make-bytes (- width size) byte) attr))
        (set! size 0)
        (printer-newline p)))))
            ...
            (let* ((ulen (~utf8-length text)) (new-size (+ current-size ulen)))
              (if (<= new-size width)
                  ...
                  (let* ((ulen   (- ulen (- new-size width)))
                         ...
                         (prefix (make-mbytes len 0)))
                    (mbytes-copy! prefix 0 text 0 len)
                    (set! size width)
                    (printer-print p (mbytes->bytes prefix) attr)))))))
    ...
    (make-printer
      (lambda (text attr)
        (let* ((ulen (~utf8-length text)) (new-size (+ size ulen)))
          (if (<= new-size width)
              (begin (set! size new-size) (printer-print p text attr))
              (let ((len.full (bytes-length text)))
                (let loop ((start 0) (ulen ulen) (new-size new-size))
                  (let* ((ulen.rhs (- new-size width))
                         (len.lhs  (~utf8-index text start (- ulen ulen.rhs)))
                         (lhs      (make-mbytes len.lhs 0)))
                    (mbytes-copy! lhs 0 text start len.lhs)
                    (printer-print p (mbytes->bytes lhs) attr)
                    (printer-newline p)
                    (let ((start (+ start len.lhs)) (ulen ulen.rhs))
                      (if (<= ulen width)
                          (let* ((len.lhs (- len.full start)) (lhs (make-mbytes len.lhs 0)))
                            (mbytes-copy! lhs 0 text start len.lhs)
                            (set! size ulen)
                            (printer-print p (mbytes->bytes lhs) attr))
...
(splicing-local
  ...
  (define (layout:pretty printer width)
    (mlet ((start* '(0)) (pos.actual 0) (pos.potential 0) (group.outer #f) (group.inner #f))
      ...
      (define (newline)
        (let* ((start (car start*))
               (text  (make-mbytes start byte:space)))
          (printer-newline printer)
          (printer-print printer (mbytes->bytes text) #f)
  ...
  (mlet ((frame* '()))
    ...
    (make-reader
      (lambda (loc text datum)
        (let ((top (frame*-top)))
          (cond ((eqv? top 'rb) (fail loc text "too many values following dot"))
                ((and (seq? top) (eqv? (seq-type top) 'bytes))
      ...
      (lambda (loc text type)
        (let ((top (frame*-top)))
          (cond ((eqv? top 'rb) (fail loc text "too many values following dot"))
                ((and (seq? top) (eqv? (seq-type top) 'bytes)) (fail loc text "not a byte"))
                ((and (seq? top) (seq-full? top)) (fail-full top loc text))
...
(splicing-local
  ((define byte:bell 7)
   ...
   (define (punctuation? c) (memv c (bytes->list #"\"#'(),;[\\]`{|}")))
  ...
  (define (make-number->utf8 notation)
    (let* ((notation  (notation-override number-notation.default notation))
      ...
      (define (integer->utf8 n)
        (cond ((< n 0) (let* ((len (+ (floor-log (- n) radix) 2)) (mbv (make-mbytes len 0)))
                         (mbytes-set! mbv 0 byte:-)
                         (set-nat-radix-digits! mbv 1 (- len 1) (- n) radix)
                         (mbytes->bytes mbv)))
              ((< 0 n) (let* ((len (+ (floor-log n radix) 1)) (mbv (make-mbytes len 0)))
                         (set-nat-radix-digits! mbv 0 len n radix)
                         (mbytes->bytes mbv)))
              (else #"0")))
      (define (ratio->utf8 n) (bytes-append (integer->utf8 (numerator n)) #"/"
            ...
            (let-values (((non-repeating-count repeating-count) (rhs-digit-counts part)))
              (if (and (not repeat?) (< 0 repeating-count))
                  ...
                  (let* ((part-len          (+ non-repeating-count repeating-count))
                         ...
                         (mbv               (make-mbytes len 0)))
                    (let* ((i (if (< n 0)
                                  (begin (mbytes-set! mbv 0 byte:-)
                                         1)
                                  0))
                           ...
                           (i (if (< 0 power-digit-count)
                                  (begin (mbytes-set! mbv i (if (<= radix 10) byte:e byte:p))
                                         (+ i 1))
                                  i))
                           (i (if (< power 0)
                                  (begin (mbytes-set! mbv i byte:-)
      ...
      (lambda (n) (let ((text (if (or (not (integer? n)) (and exp-above (< exp-above (abs n))))
                                  (fraction->utf8 n)
                                  (integer->utf8 n))))
                    (if prefix (bytes-append prefix text) text)))))
    ...
    (lambda (bv)
      (let ((len (bytes-length bv)))
        ...
        (define (Sign i b k)
          (define (next sign)
            (let ((i (+ i 1)))
              (and (< i len) (k sign i (bytes-ref bv i)))))
          (cond ((= b byte:-) (next -1))
                ((= b byte:+) (next  1))
                (else         (k 1 i b))))
            ...
            (let ((x (undigit16 b)))
              (if (< -1 x radix)
                  (let ((n (+ (* n radix) x)) (digit-count (+ digit-count 1)) (i (+ i 1)))
                    (if (< i len)
                        (loop n digit-count i (bytes-ref bv i))
                        (k.end n digit-count)))
                  (k.more n digit-count i b)))))
        (define (Digit* radix i k.more k.end)
          (if (< i len)
              (Digit*1 radix i (bytes-ref bv i) k.more k.end)
              (k.end 0 0)))
  ...
  (define (make-notate notation)
    (let* ((notation           (notation-override notation.default notation))
           ...
           (text.special-lb    #"#<")
           (text.special-rb    #">")
           (text.null          (bytes-append text.left-bracket text.right-bracket)))
      (lambda (x writer)
        (mlet ((buf.text (make-mbytes 64 0)))
          (let notate/seen ((x x) (mv* '()) (mr* '()))
            (define (notate x) (notate/seen x mv* mr*))
            ...
            (define notate-text
              (lambda (t.prefix byte:delim delim? bv len)
                (mlet ((pos.text 0))
                  (define (grow pos end)
                    (let ((len (mbytes-length buf.text)))
                      (when (< len end) (let ((new (make-mbytes (* len 2) 0)))
                                          (mbytes-copy! new 0 buf.text 0 pos)
                                          (set! buf.text new)))))
                  (define (claim len) (let* ((pos pos.text) (end (+ pos len)))
                                        (grow pos end)
                                        (set! pos.text end)
                                        pos))
                  (let ((len.t.prefix (bytes-length t.prefix)))
                    (claim (+ len.t.prefix 1))
                    (mbytes-copy! buf.text 0 t.prefix 0 len.t.prefix))
                  (let loop ((i 0) (delim? delim?))
                    (define (slash-escape b)
                      (let ((j (claim 2)))
                        (mbytes-set! buf.text j byte:backslash)
                        (mbytes-set! buf.text (+ j 1) b)
                        (loop (+ i 1) #t)))
                    (define (code-escape width prefix c)
                      (let* ((dcount     (digit16-count c))
                             (len.prefix (bytes-length prefix))
                             (j          (claim (+ dcount len.prefix 1))))
                        (mbytes-copy! buf.text j prefix 0 len.prefix)
                        (mbytes-set! buf.text (+ j dcount len.prefix) byte:semicolon)
                        (set-nat-radix-digits! buf.text (+ j len.prefix) dcount c 16)
                        (loop (+ i width) #t)))
                    (if (< i len)
                        (let* ((b0 (bytes-ref bv i)) (kf (lambda _ (code-escape 1 #"\\x" b0))))
                          (utf8-decode-width/k
                            b0 kf
                            (lambda (width)
                              (utf8-ref/b0&width/k
                                ...
                                (lambda (c)
                                  (cond ((eqv? c byte:bell)      (slash-escape byte:a))
                                        ...
                                        (else (let ((j (claim width)))
                                                (mbytes-copy! buf.text j bv i width))
                                              (loop (+ i width) (or delim? (unicode-hspace? c)
                                                                    (punctuation? c))))))))))
                        (atom (if delim?
                                  (let ((j (claim 1)) (len.t.prefix (bytes-length t.prefix)))
                                    (mbytes-set! buf.text len.t.prefix byte:delim)
                                    (mbytes-set! buf.text j            byte:delim)
                                    (mbytes->bytes buf.text 0 pos.text))
                                  (mbytes->bytes buf.text 1 (- pos.text 1)))))))))
            (cond
              ...
              ((vector? x)
               (let ((len (vector-length x)))
                 (left-bracket (bytes-append #"#" (if length-prefix? (number->utf8 len) #"")
                                             text.left-bracket))
                 (unless (= len 0)
              ...
              ((bytes? x)
               (let* ((len      (bytes-length x))
                      (t.prefix (if length-prefix? (bytes-append #"#" (number->utf8 len)) #""))
                      (len      (if (and (< 1 len) length-prefix?)
                                    (let ((last (bytes-ref x (- len 1))))
                                      (let loop ((i (- len 2)))
                                        (if (equal? (bytes-ref x i) last)
                                            (if (< 0 i) (loop (- i 1)) 1)
                                            (+ i 2))))
                                    len)))
                 (if bytes-numeric?
                     (begin (left-bracket (bytes-append t.prefix #"vu8" text.left-bracket))
                            (for-each (lambda (i) (notate (bytes-ref x i))) (range len))
                            (right-bracket text.right-bracket))
                     (notate-text t.prefix byte:dquote #t x len))))
              ((symbol? x)
               (cond ((eqv? x '||) (atom #"||"))
                     ((eqv? x '|.|) (atom #"|.|"))
                     (else (let ((bv (symbol->bytes x)))
                             (notate-text #"" byte:pipe (or (= (bytes-ref bv 0) byte:@)
                                                            (utf8->n bv))
                                          bv (bytes-length bv))))))
              ((number? x)     (atom (n->utf8 x)))
              ((mbytes? x)     (left-bracket text.special-lb)
                               (atom #"mbytes")
                               (notate (mbytes->bytes x))
                               (right-bracket text.special-rb))
              ((mvector? x)    (left-bracket text.special-lb)
                               (atom #"mvector")
                               (let loop ((i 0) (parent* mv*))
                                 (cond ((null? parent*)        (notate/seen (mvector->vector x) (cons x mv*) mr*))
                                       ((eqv? x (car parent*)) (atom (bytes-append #"#" (number->utf8 i))))
                                       (else                   (loop (+ i 1) (cdr parent*)))))
                               (right-bracket text.special-rb))
              ((procedure? x)  (let ((desc (describe x)))
                                 (if desc
                                     (begin (left-bracket text.special-lb)
                                            (atom #"procedure")
                                            (notate desc)
                                            (right-bracket text.special-rb))
                                     (atom #"#<procedure>"))))
              ((describe x) => (lambda (desc)
                                 ...
                                 (let loop ((i 0) (parent* mr*))
                                   (cond ((null? parent*)        (notate/seen desc mv* (cons x mr*)))
                                         ((eqv? x (car parent*)) (atom #"record")
                                                                 (atom (bytes-append #"#" (number->utf8 i))))
                                         (else                   (loop (+ i 1) (cdr parent*)))))
                                 (right-bracket text.special-rb)))
              (else            (atom #"#<unknown>"))))))))
  ...
  (define (make-denotate notation)
    (let ((utf8->n (make-utf8->number
      ...
      (lambda (pos.abs port reader)
        (mlet ((buf (make-mbytes 8192 0)) (pos.buf 0) (end.buf 0) (eof? #f) (pos.abs pos.abs))
          (define (buf-slice start count) (mbytes->bytes buf (+ pos.buf start) count))
          ...
          (define (suppressed-fail offset) (pos++ (max 1 offset)) #f)
          ...
          (define Text
            (mlet ((buf.text (make-mbytes 64 0)))
              (lambda (i byte:delim return/k)
                (mlet ((pos.text 0) (fail? #f) (suppress-fail? #f))
                  (define (grow pos end)
                    (let ((len (mbytes-length buf.text)))
                      (when (< len end) (let ((new (make-mbytes (* len 2) 0)))
                                          (mbytes-copy! new 0 buf.text 0 pos)
                  ...
                  (define (push c) (let* ((w (utf8-encode-width c)) (pos pos.text) (end (+ pos w)))
                                     ...
                                     (set! pos.text end)))
                  (let loop ((i i))
                    (define (fail/next i . x*)
                      (set! fail? #t)
                      (unless suppress-fail? (unless (apply fail-local x*) (set! suppress-fail? #t)))
                      (loop i))
                    ...
                    (define (k.next c w)
                      (cond ((not c) (fail i (buf-span i) "EOF before text delimiter"))
                            ...
                            ((eqv? c byte:delim)
                             (cond ((not fail?)
                                    (return/k (+ i 1) (mbytes->bytes buf.text 0 pos.text)
                                              (lambda (x) (atom (+ i 1) x))))
                                   (suppress-fail? (suppressed-fail (+ i 1)))
                                   (else (fail (+ i 1) (buf-span (+ i 1)) "invalid text"))))
          ...
          (define (Bytes i len)
            (Text i byte:dquote
                  (if len
                      (lambda (next text k)
                        (let ((len.text (bytes-length text)))
                          (cond ((< len len.text)
                                 (fail next (buf-span next) "bytes longer than length-prefix"
                                       (cons 'length len.text) (cons 'length-prefix len)))
                                ((< len.text len)
                                 (if (< 0 len.text)
                                     (let* ((b.pad (bytes-ref text (- len.text 1)))
                                            (mbv   (make-mbytes len b.pad)))
                                       (mbytes-copy! mbv 0 text 0 (- len.text 1))
                                       (k (mbytes->bytes mbv)))
                                     (fail next (buf-span next) "empty length-prefixed bytes"
                                           len)))
                                (else (k text)))))
                      (lambda (next text k) (k text)))))
          (define (String)      (Text 1 byte:dquote (lambda (_ t k) (k t))))
          (define (Symbol/pipe) (Text 1 byte:pipe   (lambda (_ t k) (k (bytes->symbol t)))))
          (define (Number^Symbol)
            (next-separator-pos 1 (lambda (i.sep)
                                    (let ((text (buf-span i.sep)))
                                      (if (eqv? (bytes-ref text 0) byte:@)
                                          (fail i.sep text "symbol cannot begin with @")
                                          (atom i.sep (or (utf8->n text) (bytes->symbol text))))))))
          (define (Number)
              ...
              (peek i (lambda (c width)
                        (let ((j (+ i width)) (n (undigit10 c)))
                          (cond (n                     (loop j (+ (* len 10) n)))
                                ...
                                (else (seq-alt*
                                        i (lambda () (fail i (buf-span+1 i) "misplaced length-prefix"))
                                        (list byte:v byte:u byte:8 byte:lparen)
                                        (lambda () (lbrack (+ i 4) 'round  'bytes len))
                                        (list byte:v byte:u byte:8 byte:lsquare)
                                        (lambda () (lbrack (+ i 4) 'square 'bytes len))
                                        (list byte:v byte:u byte:8 byte:lcurly)
                                        (lambda () (lbrack (+ i 4) 'curly  'bytes len))))))))))
          (define (Here-bytes)
            (let loop ((i 2))
              (peek i (lambda (c width)
                        (cond ((not c) (fail i (buf-span i) "EOF before start of here-bytes"))
                              ((eqv? c byte:dquote)
                               (let ((delim-length (- i 1))
                                     (delim-text (bytes-append (buf-slice 2 (- i 2)) #"#")))
                                 (let loop ((i (+ i 1)))
                                   (peek i (lambda (c width)
                                             (cond ((not c) (fail i (buf-span i) "EOF before end of here-bytes"))
                                                   ((eqv? c byte:dquote)
                                                   ...
                                                   (else (and (or (not (unicode-vspace? c)) (nline i c))
                                                              (loop (+ i width))))))))))
                              ((separator? c) (fail i (buf-span i) "separator before start of here-bytes"))
                              (else (loop (+ i width))))))))
          (define (Hash)
            (peek 1 (lambda (c _)
                      (cond ((memv c (list byte:F byte:f)) (Boolean #f))
                            ...
                            (else (seq-alt* 1 (lambda () (fail 1 (buf-span+1 1) "invalid #"))
                                            (list byte:v byte:u byte:8 byte:lparen)
                                            (lambda () (lbrack 5 'round  'bytes #f))
                                            (list byte:v byte:u byte:8 byte:lsquare)
                                            (lambda () (lbrack 5 'square 'bytes #f))
                                            (list byte:v byte:u byte:8 byte:lcurly)
                                            (lambda () (lbrack 5 'curly  'bytes #f))
          ...
          (define (Any)
            (peek 0 (lambda (c width)
                      (cond ((not c)                 (EOF pos.abs))
                            ...
                            ((eqv? c byte:backslash) (fail 0 (buf-span+1 0) "misplaced backslash"))
                            (else                    (Number^Symbol))))))
...
(define make-number->string           make-number->utf8)
(define radix->number->string         radix->number->utf8)
(define number->string                number->utf8)
(define number->string/decimal        number->utf8/decimal)
(define number->string/decimal/repeat number->utf8/decimal/repeat)
(define make-string->number           make-utf8->number)
(define radix->string->number         radix->utf8->number)
(define string->number                utf8->number)

(define (text? x) (or (bytes? x) (symbol? x)))
(define (text->bytes x)
  (cond ((bytes?  x) x)
        ((symbol? x) (symbol->bytes x))
        (else        (mistake 'text->bytes "not a symbol or bytes" x))))
  ...
  (mlet ((count -1))
    (lambda (name)
      ...
      (let ((name (if (text? name)
                      (text->bytes name)
                      (mistake 'gensym "not a symbol or bytes" name))))
        (bytes->symbol (bytes-append name #"." (number->utf8 count)))))))
