        ...
        (mlet ((buf.text (make-mbytes 64 0)))
          (let notate/seen ((x x) (mv* '()) (mr* '()))
            ...
            (cond
              ...
              ((string? x) (let ((bv (string->bytes x)))
                             (notate-text #"" byte:dquote #t bv (bytes-length bv))))
              ((symbol? x)
               (cond ((eqv? x '||) (atom #"||"))
                     ((eqv? x '|.|) (atom #"|.|"))
    ...
    (let ((utf8->n (make-utf8->number
      ...
      (lambda (pos.abs port reader)
        (mlet ((buf (make-mbytes 8192 0)) (pos.buf 0) (end.buf 0) (eof? #f) (pos.abs pos.abs))
          ...
          (define (String)      (Text 1 byte:dquote (lambda (_ t k) (k (bytes->string t)))))
...
(define (make-number->string   notation) (let ((n->utf8 (make-number->utf8 notation)))
                                           (lambda (n) (bytes->string (n->utf8 n)))))
(define (radix->number->string r)        (let ((n->utf8 (radix->number->utf8 r)))
                                           (lambda (n) (bytes->string (n->utf8 n)))))
(define (number->string                n) (bytes->string (number->utf8                n)))
(define (number->string/decimal        n) (bytes->string (number->utf8/decimal        n)))
(define (number->string/decimal/repeat n) (bytes->string (number->utf8/decimal/repeat n)))

(define (make-string->number   notation) (let ((utf8->n (make-utf8->number notation)))
                                           (lambda (s) (utf8->n (string->bytes s)))))
(define (radix->string->number r)        (let ((utf8->n (radix->utf8->number r)))
                                           (lambda (s) (utf8->n (string->bytes s)))))
(define (string->number s)               (utf8->number (string->bytes s)))

(define (string-append* x*) (bytes->string (bytes-append* (map string->bytes x*))))
(define (string-append . x*) (string-append* x*))
(define (string-join* separator x*)
  (bytes->string (bytes-join* (string->bytes separator) (map string->bytes x*))))
(define (string-join separator . x*) (string-join* separator x*))

(define (text? x) (or (bytes? x) (string? x) (symbol? x)))
(define (text->bytes x)
  (cond ((bytes?  x) x)
        ((string? x) (string->bytes x))
        ((symbol? x) (symbol->bytes x))
        (else        (mistake 'text->bytes "not a symbol, string, or bytes" x))))
  ...
  (mlet ((count -1))
    (lambda (name)
      ...
      (let ((name (if (text? name)
                      (text->bytes name)
                      (mistake 'gensym "not a symbol, string, or bytes" name))))
        (string->symbol (bytes->string (bytes-append name #"." (number->utf8 count))))))))
