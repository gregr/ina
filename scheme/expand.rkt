...
(define (bound-identifier-unique? i i*)
  (or (null? i*) (and (not (bound-identifier=? i (car i*)))
                      (bound-identifier-unique? i (cdr i*)))))
(define (bound-identifier-unique* i*)
  (or (null? i*)
      (let ((i (car i*)))
        (when (not (bound-identifier-unique? i (cdr i*)))
          (error "duplicate parameter:" (car i*)))
        (bound-identifier-unique* (cdr i*)))))
(define (param-identifier* p*)
  (cond ((null? p*) '())
        ((not (syntax-unwrap (car p*))) (param-identifier* (cdr p*)))
        ((identifier? (car p*)) (cons (car p*) (param-identifier* (cdr p*))))
        (else (error "invalid parameter:" (car p*)))))
