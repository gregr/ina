...
(define (var-names expr)
  (define (var-names* e*)
    (define (insert n ns) (if (memv n ns) ns (cons n ns)))
    (list-foldr (lambda (e ns) (list-foldr insert ns (var-names e))) '() e*))
  (cond ((e-var? expr) (list (e-var-name expr)))
        ((e-cons? expr) (var-names* (e-cons-ea* expr)))
        ((e-app? expr) (var-names* (e-app-ea* expr)))
        ((e-let? expr) (var-names* (list (e-let-arg expr) (e-let-body expr))))
        (else (error 'var-names (format "invalid expr: ~s" expr)))))
(define (var-count name expr)
  ...
  (cond ((e-var? expr) (if (eqv? name (e-var-name expr)) 1 0))
        ...
        ((e-let? expr) (+ (self (e-let-arg expr)) (self (e-let-body expr))))
    ...
    (cond
      ...
      ((s-decompose? step)
       (let* ((ps-defs (tt* (s-decompose-parts step) apps))
              (parts (car ps-defs)) (defs (cdr ps-defs)))
         (cons (if (e-cons? expr)
                 (e-cons (e-cons-c expr) parts)
                 (e-let (e-let-param expr) (caar parts) (caadr parts))) defs)))
      ((s-transient? step)
       (let* ((vs (var-names expr))
              (fname (gensym "f"))
