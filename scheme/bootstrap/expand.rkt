...
(define derived-ops
  '((not (lambda (b) (if b #f #t)))
    ...
    ;; TODO: multi-list maps.
    (map    (lambda (f xs) (foldr (lambda (x ys) (cons (f x) ys)) '() xs)))
    (andmap (lambda (f xs)
              (let loop ((last #t) (xs xs))
                (if (null? xs) last (loop (f (car xs)) (cdr xs))))))
    (ormap (lambda (f xs) (cond ((null? xs) #f)
                                ((f (car xs)) => (lambda (y) y))
                                (else (ormap f (cdr xs))))))
    (filter (lambda (p? xs)
              (cond ((null? xs) '())
