      ...
      ((pat-exist? pat)
       (pat-exist (pat-exist-ids pat)
                  (scopify (pat-exist-p pat)
                           (set-union bound (pat-exist-ids pat)))))
      ((pat-not? pat)
       (define nbound (set-subtract (bound-pattern-ids (pat-not-p pat)) bound))
       (define inner (scopify (pat-not-p pat) (set-union nbound bound)))
       (pat-not (if (set-empty? nbound) inner (pat-exist nbound inner))))
