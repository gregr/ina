...
(define (leaky-pattern-ids pat)
  (cond
    ((pat-exist? pat)
     (set-subtract (leaky-pattern-ids (pat-exist-p pat)) (pat-exist-ids pat)))
    ((pat-and? pat)
     (define l1 (leaky-pattern-ids (pat-and-c1 pat)))
     (define l2 (leaky-pattern-ids (pat-and-c2 pat)))
     (define g1 (set-subtract (bound-pattern-ids (pat-and-c1 pat)) l1))
     (define g2 (set-subtract (bound-pattern-ids (pat-and-c2 pat)) l2))
     (set-union (set-subtract l1 g2) (set-subtract l2 g1)))
    ((pat-or? pat)
     ...
     (set-union l1 (set-union l2 (set-union (set-subtract b1 b2)
                                            (set-subtract b2 b1)))))
    ((pat-cons? pat)
     (define l1 (leaky-pattern-ids (pat-cons-car pat)))
     (define l2 (leaky-pattern-ids (pat-cons-cdr pat)))
     (define g1 (set-subtract (bound-pattern-ids (pat-cons-car pat)) l1))
     (define g2 (set-subtract (bound-pattern-ids (pat-cons-cdr pat)) l2))
     (set-union (set-subtract l1 g2) (set-subtract l2 g1)))
    ((pat-segment? pat)
     (define l1 (leaky-pattern-ids (pat-segment-p pat)))
     (define l2 (leaky-pattern-ids (pat-segment-cdr pat)))
     (define g1 (set-subtract (bound-pattern-ids (pat-segment-p pat)) l1))
     (define g2 (set-subtract (bound-pattern-ids (pat-segment-cdr pat)) l2))
     (set-union (set-subtract l1 g2) (set-subtract l2 g1)))
